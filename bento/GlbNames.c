...
struct GlobalNameTbl {              /* Layout for a Global Name Table "control block":  */
  ...
  short         useCount;           /*    number of distinct users of this table        */
...
struct WriteGblCommArea{          /* cmWriteAllGlobalNames() communication area layout: */
  ...
  Boolean      reuseFreeSpace;    /*    true ==> try to reuse free space for names      */
};
...
void *cmCreateGlobalNameTbl(ContainerPtr container)
{
  ...
  t = (GlobalNameTblPtr)CMmalloc(container, sizeof(GlobalNameTbl));/* a "control block" is born!   */
...
{
  return (strcmp((char *)((GlobalNamePtr)g1)->globalName,
                 (char *)((GlobalNamePtr)g2)->globalName));
}

static int compare2(const void *g1, const void *g2)
{
  return (strcmp((char *)((GlobalNamePtr)g1)->globalName,
                 (char *)g2));
}
...
GlobalNamePtr cmCreateGlobalName(const void *table, const unsigned char *globalName,
                                 Boolean *dup)
{
                                ...
                                Boolean *dup)
{
  ...
  if (*dup) {                                                   /* possible dup...      */
    CMfree(container, createdGlobalName);                                  /* we don't need new one*/
...
GlobalNamePtr cmLookupGlobalName(const void *table, const unsigned char *globalName)
{
  GlobalNamePtr g;
  ContainerPtr  container = ((GlobalNameTblPtr)table)->container;

  /* Because of the variable length nature of type names, we malloc() a temporary name  */
  /* entry to be looked up.  This, of course, could cause a allocation failure!         */
...
#if OLD_STYLE_SYMLOOKUP
  {
	  ...
	  if ((t = (GlobalNamePtr)CMmalloc(container, sizeof(GlobalName) + strlen((char *)globalName))) != NULL) {
	    ...
	    SessionSuccess = true;                                      /* indicate malloc is ok*/
	  } else {
	    g = NULL;                                                   /* bummer!              */
	    SessionSuccess = false;                                     /* indicate bad malloc  */
	  }
  }
#else
    g = (GlobalNamePtr)cmLookupSymbol(globalName, ((GlobalNameTblPtr)table)->globalNameRoot, compare2);
    if (g && g->theValue == NULL) g = NULL;                     /* chk for deleted name */
    SessionSuccess = true;
#endif
...
TOCObjectPtr cmDefineGlobalNameObject(const ContainerPtr container, unsigned int  objectID,
                                      unsigned int  propertyID, unsigned int  typeID,
                                      unsigned char *globalName, unsigned int  generation,
                                      unsigned short flags, unsigned short objectFlags)

{
  ...
  v = cmSetImmValueBytes(container, &valueBytes, Value_GlobalName, ( CMCount ) globalName, 0);
  ...
  theObject = cmDefineObject(container, objectID, propertyID, typeID, &valueBytes,
                             generation, (unsigned short)(flags | kCMGlobalName),
  ...
  if (theObject != NULL)
    if (cmCountListCells(&theObject->propertyList) > 1 ||
      ...
      ERROR3(container,CM_err_MultGlblNames, cmltostr(theObject->objectID, 1, false, idStr),
...
static void write1GlobalName(GlobalNamePtr globalNameSymbol, CMRefCon refCon)
{
  ...
  unsigned int        nameLength, actualSize;
  CMSize			offset, zero;
  ...
  if ((theValueHdr->valueFlags & ValueGlobal) == 0) {
    ERROR1(container,CM_err_NotGlobalName, CONTAINERNAME);
  ...
  /* Don't write names that have been written before (modify) RPS/OMFI...               */

  if (omfsInt64NotEqual(theValue->value.globalName.offset, zero))
    return;
  ...
  nameLength  = strlen((char *)globalNameSymbol->globalName) + 1;
  ...
  if (CMfwrite(container, globalNameSymbol->globalName, sizeof(unsigned char), nameLength) != nameLength) {
    ERROR2(container,CM_err_BadGNameWrite, globalNameSymbol->globalName, CONTAINERNAME);
    AbortForEachGlobalName(CM_err_BadGNameWrite);
  }

  theValue->value.globalName.offset = offset;       /* define offset for global name    */

  omfsAddInt32toInt64(nameLength, &offset);         /* update logical OR physical EOF   */
  if (actualSize == 0)
...
Boolean cmWriteAllGlobalNames(const void *table)
{
  ...
  if ((container->useFlags & kCMReuseFreeSpace) == 0) {   /* if not reusing free space..*/
    CMfseek(container, zero, kCMSeekEnd);                    /* ..write to end of container*/
    gblCommArea.reuseFreeSpace = false;                   /* ...suppress seeks          */
  } else                                                  /* if updating, resue space...*/
    gblCommArea.reuseFreeSpace = (Boolean)((container->useFlags & kCMReuseFreeSpace) != 0);

  container->physicalEOF = CMgetContainerSize(container);
  return ((Boolean)(cmForEachGlobalName(table, (CMRefCon)&gblCommArea, write1GlobalName) == 0));
}
...
void cmFreeAllGlobalNames(void **table)
{
  ...
  cmFreeAllSymbols((CMContainer)container, (void **)&((GlobalNameTblPtr)*table)->globalNameRoot, SESSION);

  CMfree(container, *table);                                       /* bye, bye table               */
...
char *cmIsGlobalNameObject(TOCObjectPtr typePropertyObject, unsigned int  objectID)
{
...
char *cmGetGlobalTypeName(ContainerPtr container, unsigned int  typeID)
{
  TOCObjectPtr type;
  char         *typeName;
...
char *cmGetGlobalPropertyName(ContainerPtr container, unsigned int  propertyID)
{
