...
unsigned int  cmCountListCells(const void *theList)
{
...
Boolean cmIsEmptyList(const void *theList)
{
...
void *cmGetNthListCell(const void *theList, const unsigned int  n)
{
  unsigned int  i;
  ListLinksPtr  p;
...
void *cmInsertBeforeNthListCell(const void *theList, const void *theCell, const unsigned int  n)
{
  void *beforeThisCell;
...
void *cmInsertAfterNthListCell(const void *theList, const void *theCell, const unsigned int  n)
{
  void *afterThisCell;
...
void *cmInsertNthListCell(const void *theList, const void *theCell, const unsigned int  n)
{
...
unsigned int  cmGetCellPosition(const void *theList, const void *theCell)
{
  unsigned int  i = 0;
...
void cmFreeAllListCells(CMContainer container, const void *theList, SessionGlobalDataPtr sessionData)
{
  ListLinksPtr next, p = (ListLinksPtr)((ListHdrPtr)theList)->head;

  while (p) {                                                 /* loop through all cells */
    next = p->next;                                           /* cell is being deleted  */
    SessionFree(container, p);
    p = next;
  }
