...
void *cmLookupSymbol(const void *symbol, const void *root,
                     int (*compare)(const void *, const void *))
{
  SymbolLinksPtr p;
  int            comparison;
...
static void CM_NEAR freeSymbols(CMContainer container, const void *symbol, SessionGlobalDataPtr sessionData)
{
  void *lLink, *rLink;
  ...
  if (symbol) {                               /* if we got a symbol...                  */
    ...
    if (lLink)
      freeSymbols(container, lLink, sessionData);          /* ...follow the left link down           */

    SessionFree(container, symbol);                      /* ...free a symbol                       */

    if (rLink)
      freeSymbols(container, rLink, sessionData);          /* ...do same for the right links         */
  }
}
...
void cmFreeAllSymbols(CMContainer container, void **root, SessionGlobalDataPtr sessionData)
{
  freeSymbols(container, *root, sessionData);
  *root = NULL;
}
