 ...
 *-----------------------------------------------------------------*
 ...
 Moral -- Follow the damn rules and call CMStartSession().
*/
...
CMSession CM_FIXEDARGS CMStartSession(CMMetaHandler metaHandler, CMRefCon sessionRefCon)
{
  ...
  FreeProto            cmFree;
  FreeCtrProto		   cmFreeCtr;
  ErrorProto           cmReportError;

  /* Make the Apple lawers happy.  The following has the effect of burying a copyright  */
  ...
  if (cmReportError == NULL) return (NULL); /* if no error handler, screw it!           */
  ...
  cmFreeCtr = *(FreeCtrProto)(*metaHandler)(NULL, (CMGlobalName)CMFreeCtrOpType);

  if (cmMalloc == NULL || cmFree == NULL) { /* damn it...                               */
    (*cmReportError)(NULL,NULL,CM_err_MissingHandler);
    return (NULL);
  }

  /* Allocate the space for the session (task) global data area...                      */

  sessionData = (SessionGlobalDataPtr)(*cmMalloc)(NULL, (CMSize32)(sizeof(SessionGlobalData) + strlen(version)));
  if (sessionData == NULL) {
    (*cmReportError)(NULL,NULL,CM_err_NoSession);
    return (NULL);
  }
  ...
  sessionData->cmFreeCtr        = cmFreeCtr;            /* handlers.h                   */
  ...
  strcpy( (char*) sessionData->cmVersion, version);     /* actually no one!             */
...
void CM_FIXEDARGS CMEndSession(CMSession sessionData, CMBoolean closeOpenContainers)
{
  ...
  while (container != NULL) {                   /* look at each open container...       */
    if (closeOpenContainers)                    /* if user wants US to close it...      */
      ...
      ERROR1(container,CM_err_StillOpen, CONTAINERNAME);  /* ...YELL loudly!                      */
  ...
  /* Now that the damn containers are disposed of, we can free the session data.  This  */
  ...
  SessionFree(NULL, session);                         /* and there goes the session itself    */
}
