                        ...
                        Boolean deleteHdr)
{
  ...
  while (theValue) {
    ...
    cmAddToFreeList(container, theValue, zero, zero);           /* add space to free list     */
  ...
  if (HasRefDataObject(theValueHdr))  {                       /* delete refs object...  */
    ...
    cmMarkObjectDeleted(container, RefDataObject(theValueHdr));
  }
  #if CMSHADOW_LIST
  ...
  if (deleteHdr || !cmKeepDeletedRefNums(container->toc))     /* free the refNum?       */
    CMfree(container, theValueHdr);                                      /* this caller is serious!*/
  ...
  if (cmIsEmptyList(&theProperty->valueHdrList))
    CMfree(container, cmDeleteListCell(&theProperty->theObject->propertyList, theProperty));
}
...
void cmDeleteProperty(ContainerPtr container, TOCPropertyPtr theProperty)
{
  ...
  CMfree(container, cmDeleteListCell(&theObject->propertyList, theProperty));
}
...
static void deleteProperties(TOCObjectPtr theObject, CMRefCon refCon)
{
  ...
  while (theProperty) {
    ...
    while (theValueHdr) {
      ...
      /*CMfree(container, theValueHdr);*/                            /* no longer freed            */
      ...
      #if CMSHADOW_LIST
      if (HasRefShadowList(theValueHdr))                        /* del. refs shadow list*/
        cmDeleteRefDataShadowList(theValueHdr);
      #endif
    ...
    CMfree(container, theProperty);                                  /* property not needed        */
...
/*-----------------------------------------------------------------------------*
 | freeProperties - free ALL the properties and and their values for an object |
 *-----------------------------------------------------------------------------*

 This is an internal routine called to unconditionally free all the properties for an
...
void cmFreeTOC(ContainerPtr container, void **toc)
{
  ...
  cmDestroyTOC(toc, (void *)container, cmFreeProperties);
 ...
 *-------------------------------*
 ...
 (4). Value_Imm_Short     field according to type and, for the hell of it, the notImm
 ...
 for Value_GlobalName if the allocation fails and an error will have been reported (note,
 as usual we're covering our ass again here).
*/
/*
TOCValueBytesPtr cmSetImmValueBytes(const ContainerPtr container,
                                 TOCValueBytesPtr theValueBytes,
                                 ConstValueType dataType, unsigned int  data,
                                 unsigned int  dataLength)
{
	CMCount	data64;
	CMSize	length64;

	omfsCvtUInt32toInt64(data, &data64);
	omfsCvtUInt32toInt64(dataLength, &length64);
	return(cmSetValueBytes(container, theValueBytes, dataType, data64, length64));
}
*/
TOCValueBytesPtr cmSetImmValueBytes(const ContainerPtr container,
                                 TOCValueBytesPtr theValueBytes,
                                 ConstValueType dataType,
                                 CMCount	data64,
                                 CMSize	length64)
{
	return(cmSetValueBytes(container, theValueBytes, dataType, data64, length64));
...
TOCValueBytesPtr cmSetValueBytes(const ContainerPtr container,
                                 TOCValueBytesPtr theValueBytes,
                                 ConstValueType dataType, CMCount data,
                                 CMSize dataLength)
{
  GlobalNamePtr g;
  Boolean       dup;
  CMSize32		data32, dataLen32;

  switch (dataType) {       /* set 'em...                                               */
    case Value_NotImm:      theValueBytes->notImm.value    = data;
                            theValueBytes->notImm.valueLen = dataLength;
                            break;

    case Value_Imm_Chars:   omfsTruncInt64toUInt32(data, &data32);
    						omfsTruncInt64toUInt32(dataLength, &dataLen32);
    						if (dataLen32 > sizeof(CM_ULONG)) dataLen32 = sizeof(CM_ULONG);
                            omfsCvtUInt32toInt64(data32, &theValueBytes->imm.value);    /* should zero extra bytes */
                            omfsCvtUInt32toInt64(dataLen32, &theValueBytes->notImm.valueLen);
                            break;

    case Value_Imm_Long:    omfsTruncInt64toUInt32(data, &data32);
                            omfsCvtUInt32toInt64(data32, &theValueBytes->imm.value);
                            omfsCvtUInt32toInt64(sizeof(CM_ULONG), &theValueBytes->notImm.valueLen);
                            break;

    case Value_Imm_Short:   omfsTruncInt64toUInt32(data, &data32);
                            omfsCvtUInt32toInt64(0, &theValueBytes->imm.value);     /* zero extra bytes */
                            omfsCvtUInt32toInt64(data32, &theValueBytes->imm.value);
                            omfsCvtUInt32toInt64(sizeof(CM_USHORT), &theValueBytes->notImm.valueLen);
                            break;

    case Value_Imm_Byte:    omfsTruncInt64toUInt32(data, &data32);
                            omfsCvtUInt32toInt64(data32, &theValueBytes->imm.value);
                            omfsCvtUInt32toInt64(sizeof(CM_UCHAR), &theValueBytes->notImm.valueLen);
                            break;

    case Value_GlobalName:  omfsTruncInt64toUInt32(data, &data32);

                            /* JeffB: Clear out the entire structure to eliminate purify hits
                             */
                            omfsCvtUInt32toInt64(0, &theValueBytes->imm.value);
                            omfsCvtUInt32toInt64(0, &theValueBytes->notImm.valueLen);
    						/* !!! */
    						g = cmCreateGlobalName(container->globalNameTable, (unsigned char *)data, &dup);
                            if (g == NULL) {
                              ERROR2(container,CM_err_NoGlobalName, data32, CONTAINERNAME);
                              theValueBytes = NULL;
                            } else if (dup) {
                              ERROR2(container,CM_err_DupGlobalName, data32, CONTAINERNAME);
                              theValueBytes = NULL;
                            } else {
                              theValueBytes->globalName.globalNameSymbol = g;
                              omfsCvtUInt32toInt64(0, &theValueBytes->globalName.offset);
                            }
                            break;
  } /* switch */
...
unsigned short cmSetTOCValueHdrFlags(TOCValueHdrPtr theValueHdr, const unsigned short flags)
{
  unsigned short valueFlags = (unsigned short)(theValueHdr->valueFlags & ValueCstFlags);
  ...
  if (flags & kCMGlobalName)                        /* global names                     */
    ...
    ERROR1(container,CM_err_Internal1, cmltostr(flags, -4, true, flagsStr));
                                 ...
                                 const unsigned short flags)
{
  ...
  if ((theValue = (TOCValuePtr)CMmalloc(container, sizeof(TOCValue))) == NULL) { /* create seg...  */
    ERROR1(container,CM_err_NoValueEntry, CONTAINERNAME);
    return (NULL);
  }
  ...
  omfsCvtUInt32toInt64(0, &theValue->logicalOffset);   /* filled in elsewhere            */
                          ...
                          const unsigned short flags)
{
  ...
  endOff = value->notImm.value;
  omfsAddInt64toInt64(value->notImm.valueLen, &endOff);
  if (!container->tocFullyReadIn && (flags & kCMImmediate) == 0 &&
      (omfsInt64Greater(value->notImm.value, container->maxValueOffset)          ||
       omfsInt64Greater(endOff, container->maxValueOffset))) {
    ERROR3(container,CM_err_BadOffset, cml64tostr(value->notImm.value, 1, false, offsetStr),
                             cml64tostr(value->notImm.valueLen, 1, false, lenStr), CONTAINERNAME);
    return (NULL);
  }
  ...
  if ((theValueHdr->valueFlags & ValueGlobal) != 0)   /* global name                    */
    omfsAddInt32toInt64(GetGlobalNameLength(theValue->value.globalName.globalNameSymbol) + 1, &theValueHdr->size);
  else if (theValue->flags & kCMImmediate)
    omfsAddInt64toInt64(theValue->value.notImm.valueLen, &theValueHdr->size); /* immediate byte, word, int  */
  else
    omfsAddInt64toInt64(theValue->value.notImm.valueLen, &theValueHdr->size); /* non-immediate data         */
...
static TOCValueHdrPtr CM_NEAR defineValue(ContainerPtr container,
                                          ...
                                          Boolean *newValueHdr)
{
  TOCValueHdrPtr theValueHdr, testHdr;
  ...
  /* value. Having said all this, it should be noted that all this continue crap is     */
  ...
  if ((theValueHdr = (TOCValueHdrPtr)cmGetListTail(&theProperty->valueHdrList)) != NULL &&
    ...
    if (theValueHdr->typeID != typeID) {              /* ...types must agree            */
      ERROR1(container,CM_err_BadContinue, CONTAINERNAME);
      return (NULL);
    }
    *newValueHdr = false;                             /* ...use most recent value hdr   */
    ...
    if ((theValueHdr = (TOCValueHdrPtr)CMmalloc(container, sizeof(TOCValueHdr))) == NULL) {
      ERROR1(container,CM_err_NoValueEntry, CONTAINERNAME);
      return (NULL);
    }
    ...
    *newValueHdr = true;
    ...
    omfsCvtUInt32toInt64(0, &theValueHdr->size);       /* ...total value size initially 0*/
    omfsCvtUInt32toInt64(0, &theValueHdr->logicalSize);  /* ...same for logicalSize        */
  ...
  /* Test for existing values, as this test is quick and should happen rarely
   * (multiple values for a property is rare).  There should NEVER be two values
   * of the same type, as Bento only knows how to do a CMUseValue to the first one,
   * and the property iterator gets into an infinite loop.
   */
  if(((flags & ValueContinued) == 0) && !cmIsEmptyList(&theProperty->valueHdrList))
  {
	testHdr = (TOCValueHdrPtr)cmGetListHead(&theProperty->valueHdrList);
	while (testHdr)
	{                                 /* scan each value hdr on list  */
	    if ((testHdr->typeID == typeID) && ((testHdr->valueFlags & ValueContinued) == 0))
	    {
	    	value = NULL;
	    	break;          /* Can't have two values of the same type   */
	    }
	    testHdr = (TOCValueHdrPtr)cmGetNextListCell(testHdr);
	}
  }
...
static TOCPropertyPtr CM_NEAR defineProperty(ContainerPtr container,
                                             ...
                                             TOCValueBytesPtr value,
                                             const unsigned int  generation,
                                             const unsigned short flags, Boolean *newProperty,
                                             TOCValueHdrPtr *theValueHdr)
{
  ...
  /* Scan all the properties for the object to see if we got a new one...               */
...
#endif
	{
		theProperty = cmGetObjectProperty(theObject, propertyID);
  		*newProperty = (Boolean)(theProperty == NULL);
  	}

  /* If we got a new property create it and init its fields...                          */

  if (*newProperty) {                             /* new property...                    */
    if ((theProperty = (TOCPropertyPtr)CMmalloc(container, sizeof(TOCProperty))) == NULL) {
      ERROR1(container,CM_err_NoPropEntry, CONTAINERNAME);
      return (NULL);
    }
  ...
  *theValueHdr = defineValue(container, theProperty, typeID, value, generation, flags, &newValueHdr);
  ...
  if (*theValueHdr == NULL) {
    if (*newProperty) CMfree(container, theProperty);
    return (NULL);
  }
...
TOCObjectPtr cmDefineObject(ContainerPtr container, unsigned int  objectID,
                            const unsigned int  propertyID, const unsigned int  typeID,
                            TOCValueBytesPtr value, const unsigned int  generation,
                            ...
                            TOCValueHdrPtr *theValueHdr)
{
	...
	omfInt64	zero;
	omfsCvtUInt32toInt64(0, &zero);
	return(cmIntDefineObject(container, objectID, propertyID, typeID,  value, generation,
                            flags, objectFlags, LoadAllFields, zero, 0, theValueHdr));
}

TOCObjectPtr cmIntDefineObject(ContainerPtr container, unsigned int  objectID,
                            const unsigned int  propertyID, const unsigned int  typeID,
                            TOCValueBytesPtr value, const unsigned int  generation,
                            const unsigned short flags, const unsigned short objectFlags,
                            omfObjectLoadType_t loadType, CMCount filePos, unsigned int  entrySize,
                            TOCValueHdrPtr *theValueHdr)
{
#endif
  TOCObjectPtr   theObject;
  ...
  if (objectID == 0x00000000UL) {
    ERROR1(container,CM_err_wrappedIDs, CONTAINERNAME);
    return (NULL);
  }
  ...
  if (theObject == NULL) {
    ERROR1(container,CM_err_NoObjEntry, CONTAINERNAME);
    return (NULL);
  }
  ...
  if (objectFlags & UndefinedObject) {            /* do what it says above!             */
    ...
    if (theObject->objectFlags & (PropertyObject | TypeObject)) {         /*    (4)     */
      ERROR2(container,CM_err_MultDef, cmltostr(objectID, 1, false, idStr), CONTAINERNAME);
      return (NULL);
    }
    theObject->objectFlags = (unsigned short)(objectFlags & ~UndefinedObject);
  ...
  theObject->objectFlags &= ~UndefinedObject;
...
#ifdef VIRTUAL_BENTO_OBJECTS
	/* JeffB: Add change to support virtual objects */
	if(loadType == LoadAllFields)
	{
#endif
	  ...
	  if (newProperty)
	  	cmAppendListCell(&theObject->propertyList, theProperty);
#ifdef VIRTUAL_BENTO_OBJECTS
	...
	else
	{
		theObject->objectFlags |= ObjectIsNotLoaded;
		if(!dup)
			omfsCvtInt32toInt64(0, &theObject->allEntrySize);
		if(omfsInt64Equal(theObject->allEntrySize, zero))
			theObject->fileOffset = filePos;
		omfsAddInt32toInt64(entrySize, &theObject->allEntrySize);
	}
#endif
  ...
  if (container->tocFullyReadIn)
	  if (typeID != CM_StdObjID_ObjRefData)    /* if not a recording value...*/
...
TOCPropertyPtr cmGetObjectProperty(TOCObjectPtr theObject, unsigned int  propertyID)
{
  TOCPropertyPtr theProperty;
	...
	if(theObject->objectFlags & ObjectIsNotLoaded)
	{
		if(cmLoadUpStubObject(theObject) == NULL)
			return(NULL);
	}
#endif
  theProperty = (TOCPropertyPtr)cmGetListHead(&theObject->propertyList);
...
TOCValueHdrPtr cmGetPropertyType(TOCPropertyPtr theProperty, unsigned int  typeID)
{
  TOCValueHdrPtr theValueHdr;
...
CMSize cmGet1ValueSize(TOCValuePtr theValue)
{
 CMSize size;

  if (theValue->flags & kCMGlobalName)
    omfsCvtUInt32toInt64(GetGlobalNameLength(theValue->value.globalName.globalNameSymbol) + 1, &size);
  else if (theValue->flags & kCMImmediate)
    size = theValue->value.notImm.valueLen;       /* immediate byte, word, or int  data */
  else
...
Boolean cmValidateCMValue(CMValue value)
{
  ContainerPtr container;
  ...
  if ((((TOCValueHdrPtr)value)->valueFlags & ValueDeleted) != 0) {
    ERROR1(container,CM_err_BadValue, CONTAINERNAME);
    return (false);
  }
...
Boolean cmValidateCMObjects(CMObject object, unsigned short objectFlags)
{
  ContainerPtr container;
  ...
  if ((((TOCObjectPtr)object)->objectFlags & DeletedObject) != 0) {
    ERROR1(container,CM_err_BadObject, CONTAINERNAME);
    return (false);
  }
  ...
  if ((((TOCObjectPtr)object)->objectFlags & objectFlags) == 0) {
    ERROR2(container,CM_err_BadObjectKind, (objectFlags & TypeObject) ? "type" : "property", CONTAINERNAME);
    return (false);
  }
