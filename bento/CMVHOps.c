...
CMCount32 CM_VARARGS CMScanDataPacket(CMType type, CMMetaData metaData,
                                    CMDataPacket dataPacket, ...)
{
  ...
  nbrAssigned = cmVScanDataPacketGuts((TOCObjectPtr)type, (char *)metaData,
                                      (unsigned char *)dataPacket, dataInitParams);
  va_end(dataInitParams);

  return (nbrAssigned);                    /* return nbr of values assigned    */
}
...
CMCount32 CM_FIXEDARGS CMVScanDataPacket(CMType type, CMMetaData metaData,
                                       CMDataPacket dataPacket,
                                       va_list dataInitParams)
{
  unsigned int  nbrAssigned;
