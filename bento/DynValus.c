 ...
 *----------*  +->| (for these values) |           +-->|   (dynamic values)  |
 ...
 size specification sequences are as follows:
    ...
    l[d]      A int  (the "d" is optional) [note 1].
 ...
 int  winded explanation, but there is no short way to describe this stuff.  Would you
 rather I didn't?
*/
...
#define Meta_Long       'l'           /* int  ("%ld" also allowed)                      */
#define Meta_LongSuffix 'd'           /* optional "int " suffix                         */
                                                ...
                                                CMHandlerAddr *metaDataHandler)
{
  ...
  metaHandler = cmLookupMetaHandler((unsigned char *)typeName, container->sessionData);
  ...
  *newValueHandler = (CMHandlerAddr)(*metaHandler->metaHandler)(NULL, (CMGlobalName)CMNewValueOpType);
  if (*newValueHandler == NULL) {             /* return "new value" handler if we got it*/
    ERROR1(container,CM_err_NoNewValueHandler,typeName);/* ...yell again                          */
  ...
  *metaDataHandler = (CMHandlerAddr)(*metaHandler->metaHandler)(NULL, (CMGlobalName)CMDefineMetaDataOpType);
  if (*metaDataHandler == NULL) {             /* return "metadata" handler if we got it */
    ERROR1(container,CM_err_NoMetaDataHandler,typeName);/* ...yell again and again                */
    SessionSuccess = false;
    return (NULL);
  }
...
#define VA_COPY( s,d ) d = s
#else
#define VA_COPY( s,d ) va_copy( d,s )
#endif
...
static unsigned char *CM_NEAR createDataPacket(TOCObjectPtr type, char *metaData, va_list *constructorData)
{
  ...
  char           *mp, m, *typeName, insert[10];
  unsigned int   packetSize, l;
  unsigned short d;
  CMSize32         n;
  void           *p;
  va_list        ap;
  ...
  while ((Boolean)(m = *mp++)) {          /* scan the metaData...                       */
    ...
    switch ((Boolean)(m = *mp++)) {
      ...
      case Meta_Char:   c = va_arg(ap, unsigned char);        /* %c                     */
                        packetSize += sizeof(unsigned char);
                        continue;

      case Meta_Short:  d = va_arg(ap, unsigned short);       /* %d                     */
                        packetSize += sizeof(unsigned short);
                        continue;

      case Meta_Long:   if (*mp == Meta_LongSuffix) ++mp;     /* %l[d]                  */
      case Meta_ID:     l = va_arg(ap, unsigned int );        /* %i                     */
      ...
      case Meta_Star:   if ((m = *mp++) != Meta_String) {     /* %*s                    */
                          ...
                          ERROR3(container,CM_err_BadMetaSpec, insert, typeName, CONTAINERNAME);
                          return (NULL);
                        }
                        n = va_arg(ap, CMSize32);
                        s = va_arg(ap, unsigned char *);
                        packetSize += n + sizeof(CMSize);
                        continue;

      case Meta_String: s = va_arg(ap, unsigned char *);      /* %s                     */
                        packetSize += strlen((char *)s) + 1;
                        continue;
  ...
  dataPacket = (unsigned char *)CMmalloc(container, packetSize + 1); /* we add a null to the end   */
  if (dataPacket == NULL) {
    typeName = cmIsGlobalNameObject(type, CM_StdObjID_GlobalTypeName);
    ERROR2(container,CM_err_NoDataPacket, typeName, CONTAINERNAME);
    SessionSuccess = false;
    return (NULL);
  }
  ...
  while ((Boolean)(m = *mp++)) {          /* rescan the metaData...                     */
    ...
    switch ((Boolean)(m = *mp++)) {
      ...
      case Meta_Char:   c = va_arg(ap, unsigned char);        /* %c                     */
                        memmove(dp, &c, sizeof(unsigned char));
                        dp += sizeof(unsigned char);
                        continue;

      case Meta_Short:  d = va_arg(ap, unsigned short);       /* %d                     */
                        memmove(dp, &d, sizeof(unsigned short));
                        dp += sizeof(unsigned short);
                        continue;

      case Meta_Long:   if (*mp == Meta_LongSuffix) ++mp;     /* %l[d]                  */
      case Meta_ID:     l = va_arg(ap, unsigned int );        /* %i                     */
                        memmove(dp, &l, sizeof(unsigned int ));
                                              ...
                                              char *newOrUseValueName)
{
  ...
  if (metaHandler == NULL) {                        /* we must have a metahandler!      */
    typeName = cmIsGlobalNameObject(type, CM_StdObjID_GlobalTypeName);
    ERROR2(container,CM_err_NoMetahandler, typeName, CONTAINERNAME);
    return (NULL);
  }
  ...
  firstDynamicValue = (Boolean)(theRealValueHdr->dynValueData.dynValue == NULL);

  if (!firstDynamicValue &&                         /* do the base check...             */
      theRealValueHdr->dynValueData.dynValue != baseValueHdr) {
    ERROR2(container,CM_err_BadRealValue, newOrUseValueName, CONTAINERNAME);    /* ...oops!       */
    return (NULL);
  }
  ...
  if ((extensions = (DynValueHdrExtPtr)CMmalloc(container, sizeof(DynValueHdrExt))) == NULL) {
    typeName = cmIsGlobalNameObject(type, CM_StdObjID_GlobalTypeName);
    ERROR2(container,CM_err_NoDynExtensions, typeName, CONTAINERNAME);
    return (NULL);
  }
...
void cmDeleteAllDynamicValueLayers(TOCValueHdrPtr theValueHdr, Boolean freeValueHdr)
{
  ...
  if (IsDynamicValue(theValueHdr)) {
    ...
    while (IsDynamicValue(theValueHdr)) {             /* free all these dyn values...   */
      ...
      CMfree(container, DYNEXTENSIONS(theValueHdr));             /* ...free the extensions...      */
      CMfree(container, theValueHdr);                            /* ...and the header itself       */
                             ...
                             Boolean isNewValue, va_list *constructorData)
{
  ...
  if (baseTypeProperty != NULL) {             /* process this type's base types         */
    ...
    if (theValueHdr != NULL) {                /* double check for value hdr             */
      ...
      while (theValue != NULL) {
        ...
        baseType = cmFindObject(container->toc, aValue);
        if (baseType) {
  ...
  if (useValueHandler) {                      /* if dynamic value layer is to be created*/
    if (isNewValue) {                         /* extra stuff for CMNewValue()...        */
      ...
      metaData = (*(char *(*)(CMType))metaDataHandler)((CMType)type);
      ...
      if (dataPacket != NULL) CMfree(container, dataPacket);               /* free the packet      */
    ...
    if (success) {
      ...
      if (success) {
        if (metaHandler == NULL) {            /* we must get a methandler back!         */
          typeName = cmIsGlobalNameObject(type, CM_StdObjID_GlobalTypeName);
          ERROR2(container,CM_err_NoDynMetahandler, typeName, CONTAINERNAME);
          success = false;
        } else
                               ...
                               CMconst_CMGlobalName operationType, char *routineName)
{
  ...
  if (vectorEntry->active) {                        /* error if recursing...            */
    ...
    ERROR2(container,CM_err_HandlerRecursed, routineName, CONTAINERNAME);
    return (NULL);
  }
  ...
  if (value != origValue) {                         /* if "inherited"...                */
    entryOffset = (char *)vectorEntry - (char *)DYNEXTENSIONS(origValue); /* relocate...*/
    vectorEntry  = (DynamicValueVectorEntriesPtr)((char *)DYNEXTENSIONS(value) + entryOffset);
...
unsigned int  cmVScanDataPacketGuts(TOCObjectPtr type, char *metaData,
                                    unsigned char *dataPacket, va_list dataInitParams)
{
  ContainerPtr   container = type->container;
  unsigned char  *dp, *p;
  char           *mp, m, *typeName, insert[10];
  unsigned int   nbrAssigned;
  CMSize32         n;
  ...
  while ((Boolean)(m = *mp++))  {                 /* scan the encoding...               */
    if (m != Meta_Trigger) continue;              /* if not format specification, loop  */

    p = va_arg(dataInitParams, unsigned char *);  /* point at next parameter            */
    ...
    switch ((Boolean)(m = *mp++)) {
      ...
      case Meta_Char:   memmove(p, dp, sizeof(unsigned char));  /* %c                   */
                        dp += sizeof(unsigned char);
                        ++nbrAssigned;
                        break;

      case Meta_Short:  memmove(p, dp, sizeof(unsigned short)); /* %d                   */
                        dp += sizeof(unsigned short);
                        ++nbrAssigned;
                        continue;

      case Meta_Long:   if (*mp == Meta_LongSuffix) ++mp;       /* %l[d]                */
      case Meta_ID:     memmove(p, dp, sizeof(unsigned int ));  /* %i                   */
                        dp += sizeof(unsigned int );
                        ++nbrAssigned;
                        continue;
      ...
      case Meta_Star:   if ((m = *mp++) != Meta_String) {       /* %*s                  */
                        ...
                        dp += sizeof(CMSize);
                        ++nbrAssigned;
                        p = va_arg(dataInitParams, unsigned char *);
