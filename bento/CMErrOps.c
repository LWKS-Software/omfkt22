...
static unsigned int  CM_NEAR my_strtoul(char *s, char **endptr)
{
...
char CM_PTR * CM_VARARGS CMAddMsgInserts(char CM_PTR *msgString, CMSize32 maxLength, ...)
{
  va_list inserts;
...
char CM_PTR * CM_FIXEDARGS CMVAddMsgInserts(char CM_PTR *msgString, CMSize32 maxLength,
                                            va_list inserts)
{
  char          c, *src, *dst, *p, *insert, msgString0[MaxMsgLen];
  unsigned int  cnt = (unsigned int )maxLength;
  int           n;
  va_list       inserts0;
  ...
  while (((Boolean)(c = *src++)) && cnt > 0) {/* scan src buffer for "^n"s...           */
    ...
    VA_COPY( inserts0, inserts );             /* get n'th insert (there better be one!) */
    while (n-- >= 0)                          /* there's no way to chk that there isn't */
      insert = va_arg(inserts, char *);
...
char CM_PTR * CM_FIXEDARGS CMVAddMsgInserts(char CM_PTR *msgString, CMSize32 maxLength,
                                            va_list inserts)
{
  char    *s, *p, *q, *x, *last, *insert, c;
  int     n;
  short   msglen, inslen, signalLen, cnt;
...
*/
/*** OMF Note: Changed name to omfCMGetErrorString to avoid name
  ** clash with Metrowerks Code Warrior on the Macintosh
  */
CMErrorString CM_VARARGS omfCMGetErrorString(CMErrorString errorString, CMSize32 maxLength,
...
CMErrorString CM_FIXEDARGS CMVGetErrorString(CMErrorString errorString, CMSize32 maxLength,
                                             CMErrorNbr errorNumber, va_list inserts)
{
  ...
  #define e0(n, s)        case n:  msgStr = (char *)s; gotInserts = false; break
