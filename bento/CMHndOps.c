                                            ...
                                            CMMetaHandler metaHandler)
{
  ...
  h = cmDefineMetaHandler(metaHandler, (unsigned char *)typeName, &dup,
                                            ...
                                            CMconst_CMGlobalName typeName)
{
  ...
  h = cmLookupMetaHandler((unsigned char *)typeName, (SessionGlobalDataPtr)sessionData);
...
{ 
  ...
  if (getOpComArea.nbrOfGlobalNames > 1) {          /* must have exactly 1 global name  */
    ERROR2(container,CM_err_AmbiguousType, "CMGetOperation", CONTAINERNAME);
    return (NULL);
  }

  if (getOpComArea.nbrOfGlobalNames == 0) {
    ERROR2(container,CM_err_TypeNotGlobal, "CMGetOperation", CONTAINERNAME);
    return (NULL);
  }
  ...
  if ((metaHandler = cmLookupMetaHandler((unsigned char *)typeName, SESSION)) == NULL) { 
...
void CM_PTR * CM_FIXEDARGS CMMalloc(CMContainer container, CMSize32 size, CMSession sessionData)
{
  return ((sessionData == NULL) ? NULL : SessionMalloc(container, size));
}
...
void CM_FIXEDARGS CMFree(CMContainer container, CMPtr ptr, CMSession sessionData)
{
  if (sessionData != NULL) SessionFree(container, ptr);
}
...
void CM_VARARGS omfCMError(CMSession sessionData, CMErrorString message, ...)
{
  va_list inserts;
