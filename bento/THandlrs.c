 ...
 *---------------------------------------------------------------------------*
 ...
 container.     XHandlrs.c     is such a set.
 ...
 will use the handlers for that container (i.e., "real" handlers like those of
    XHandlrs.c    ).  For dynamic values, it would be the dynamic value-s handler package.
 ...
 opened for reading, and thus no writing handlers are needed.  You should be looking at
    XHandlrs.c    , not this file, for the documentation on how to write container
 handlers.

 There is one final thing to point out here.  If you compare the code in this file with
 the code for embedded handlers,        XEmbeded.c        , you will see they are very
 similar.  Indeed,        XEmbeded.c         was used as the basis for this file!  Here,
 ...
 *----------------------------------------------------------------*
 ...
 struct RefCon {                                  /* The refCon layout:                 */
   ...
   unsigned int  position;                        /*    current position within value   */
...
static CMRefCon open_Handler(CMRefCon attributes, CMOpenMode mode)
{
  ...
  if (typeName == NULL) {                       /* no global name?                      */
    omfCMError(sessionData, "Unable to get type name for pointing value in container \"^0\"!", CMReturnContainerName(container));
    return (NULL);
  }

  /* Allocate and fill in the refCon we will return to the Container Manager...         */

  p = (RefConPtr)CMMalloc(container, sizeof(RefCon) + strlen(typeName), sessionData);
  if (p == NULL) {                              /* allocation failed!                   */
    omfCMError(sessionData, "Allocation of \"refCon\" failed for target (type \"^0\", container \"^1\")!", typeName, CMReturnContainerName(container));
    return (NULL);
  }
  ...
  if (strcmp((char *)mode, "wb+") == 0) {       /* writing...                           */
    if (p->size != 0) {                         /* size must be zero                    */
      omfCMError(sessionData, "Cannot create target container for a pointing value (type \"^0\") that already has data!", p->typeName);
      CMFree(container, p, sessionData);
      return (NULL);
    }
    p->position = 0;                            /* position to 1st free byte            */
  } else if (strcmp((char *)mode, "rb") == 0) { /* reading...                           */
    if (p->size == 0) {                         /* size must be non-zero                */
      omfCMError(sessionData, "Cannot read target container for a pointing value (type \"^0\") that doesn't have data!", p->typeName);
      CMFree(container, p, sessionData);
      return (NULL);
    }
    ...
    omfCMError(sessionData, "Invalid target container open mode (\"^0\", pointing value type \"^1\")!", (char *)mode, p->typeName);
    CMFree(container, p, sessionData);
    return (NULL);
  }
...
static void close_Handler(CMRefCon refCon)
{
  ...
  CMFree(container, p, sessionData);                               /* bye, bye, refcon...          */
}
...
static CMSize seek_Handler(CMRefCon refCon, CM_LONG posOff, CMSeekMode mode)
{
  ...
  if (mode == kCMSeekSet)                         /* kCMSeekSet...                      */
    p->position = (unsigned int )posOff;          /* ...just use posOff as is           */
  else if (mode == kCMSeekEnd)                    /* kCMSeekEnd...                      */
    p->position = (int )p->size + posOff;         /* ...posOff is 0 or negative here    */
  else                                            /* kCMSeekCurrent...                  */
    p->position = (int )p->position + posOff;     /* ...relative positioning            */
...
static CMSize read_Handler(CMRefCon refCon, CMPtr buffer, CMSize elementSize, CMCount theCount)
{
  ...
  amountRead = (unsigned int )CMReadValueData(p->value,         /* read value...        */
                                              buffer,
                                              p->position,
                                              (CMSize)((unsigned int )elementSize * (unsigned int )theCount));

  p->position += amountRead;                    /* update position by amount read       */
  amountRead  /= (unsigned int )elementSize;    /* convert back to elementSize units    */
 ...
 *---------------------------------------------------------------*
 ...
 which the    XHandlrs.c     handlers maintain, we will put and look for the label at the
                              ...
                              CMSize *tocOffset, CMSize *tocSize)
{
  ...
  seek_Handler(refCon, -(int )sizeof(ContainerLabelFmt), kCMSeekEnd);
  labelSize = (unsigned int )read_Handler(refCon, (CMPtr)&theLabel,
                                          (CMSize)sizeof(unsigned char),
                                          (CMCount)sizeof(ContainerLabelFmt));

  if (labelSize != sizeof(ContainerLabelFmt)) {   /* must have read it all!             */
    omfCMError(p->sessionData, "Target container (for pointing value type \"^0\") label could not be read!", p->typeName);
    return;
  }
