...
static CMRefCon open_Handler(CMRefCon attributes, CMOpenMode mode)
{
  ...
  p->size = (unsigned int )CMGetValueSize(p->parentValue);
...
static void CM_NEAR displayIOBuffer(CMRefCon refCon, unsigned int  fileOffset, CMPtr buffer,
...
{
  MyRefConPtr   p = (MyRefConPtr)refCon;
  unsigned int  fileOffset, amountRead;

  if (p->tracing) fileOffset = p->position;     /* tracing (need initial offset)        */

  amountRead = (unsigned int )CMReadValueData(p->parentValue,   /* read value...        */
                                              buffer,
                                              p->position,
...
{
  MyRefConPtr   p = (MyRefConPtr)refCon;
  unsigned int  amountWritten = (unsigned int )elementSize * (unsigned int )theCount;
...
static CMBoolean trunc_Handler(CMRefCon refCon, CMSize containerSize)
{
  ...
  CMDeleteValueData(p->parentValue,                     /* delete excess stuff          */
                    ...
                    (CMSize)(p->size - (unsigned int )containerSize));

  p->size = (unsigned int )containerSize;               /* set new size                 */
                               ...
                               CMSize tocOffset, CMSize tocSize)
{
  ...
  theLabel.tocOffset    = (unsigned int )tocOffset;
  theLabel.tocSize      = (unsigned int )tocSize;
  ...
  labelSize = (unsigned int )write_Handler(refCon, (CMPtr)&theLabel,
                                           (CMSize)sizeof(unsigned char),
                                           (CMCount)sizeof(ContainerLabelFmt));
