...
CMCount32 CM_FIXEDARGS CMCountValues(CMObject object, CMProperty property, CMType type)
{
  ...
  if (type != NULL) {
    ...
    if (container->targetContainer != ((TOCObjectPtr)property)->container->targetContainer ||
      ...
      ERROR3(container,CM_err_3Containers, CONTAINERNAMEx(container),
    ...
    return ((CMCount32)(theValueHdr != NULL));
  }
  ...
  return (theProperty ? (CMCount32)cmCountListCells(&theProperty->valueHdrList) : 0);
}
 ...
 *--------------------------------------------------------------------------*
 ...
 This routine returns the refNum for the next type value (according to the current value
 order) in the objects property following currValue.  If currValue is NULL, the refNum for
 the first value for that object's property is returned.  If  currValue is not NULL, the
 next value for that object's property is returned.  NULL is returned if there are no more
 type values following currValue or the object does not contain the property.
 ...
 object as int  as no other operations change the value order.
...
{
  TOCPropertyPtr theProperty;
  TOCValueHdrPtr theValueHdr, savedValueHeader;
  ContainerPtr   container;
  CMType         type;
  ...
  /* For the first time, return the first value on the property's list...               */
  ...
  if (theValueHdr != NULL) {                        /* just to be safe!                 */
    ...
    if(theValueHdr != savedValueHeader)
    	theValueHdr = NULL;
  }
...
CMValue CM_FIXEDARGS CMGetBaseValue(CMValue value)
{
  ...
  if (container->getBaseValueOk <= 0) {             /* only callable from a handler     */
    ERROR1(container,CM_err_CantGetBase, CONTAINERNAME);
    return (NULL);
  }
...
CMSize CM_FIXEDARGS CMGetValueSize(CMValue value)
{
    ...
    GetDynHandlerAddress(value, cmGetValueSize, CMGetValueSizeOpType, "CMGetValueSize", zero);
    if (IsDynamicValue(value)) {
      SignalDynHandlerInUse(value, cmGetValueSize);
      AllowCMGetBaseValue(((TOCValueHdrPtr)value)->container);
      size = CMDynGetValueSize(value);
  ...
  return (((TOCValueHdrPtr)value)->size);   /* return size from the value hdr   */
}
...
CMSize32 CM_FIXEDARGS CMReadValueData(CMValue value, CMPtr buffer, CMCount offset, CMSize32 maxSize)
{
  TOCValueHdrPtr theValueHdr;
  TOCValuePtr    theValue;
  ContainerPtr   container;
  unsigned char  *p;
  CMSize32		  len, remaining, totalRead, amountRead;
  CMSize		lenTmp;
  ...
  if (IsDynamicValue(value)) {                      /* process dynamic value...         */
    ...
    if (IsDynamicValue(value)) {
      ...
      totalRead = CMDynReadValueData(value, buffer, offset, maxSize);
  ...
  if (container->useFlags & kCMWriting) {           /* make sure we're opend for reading*/
    ERROR1(container,CM_err_ReadIllegal, CONTAINERNAME);
    return (0);
  }
  #endif
  ...
  if (cmIsEmptyList(&theValueHdr->valueList)) {     /* must have a value                */
    ERROR1(container,CM_err_HasNoValue, CONTAINERNAME);
    return (0);
  }
  ...
  p         = (unsigned char *)buffer;              /* p points to next byte to read to */
  remaining = (unsigned int )maxSize;               /* remaining bytes to read          */
  totalRead = 0;     					          /* how much we actually do read     */

  while (theValue && remaining) {                   /* read value (segments)...         */
    lenTmp = cmGet1ValueSize(theValue);
    omfsSubInt64fromInt64(offset, &lenTmp);       /* (note, immediates work here)     */
    omfsTruncInt64toUInt32(lenTmp, &len);
    /* !!! Catch errors here */
    if (len > remaining)
    	len = remaining;         					 /* (note, so do global names)       */
    ...
    omfsCvtUInt32toInt64(0, &offset);                  /* full segments from now on      */
...
/*-------------------------------------------------------------------*
 | CMGetValueDataOffset - Get the underlying file offset for a value |
 *-------------------------------------------------------------------*

 The actual file position of the value data is returned, but only if there is one
 segment.

 It is an error to attempt to read a value which has no data, i.e., a value where only a
 CMNewValue() has been done.

 0xffffffff is returned on error
*/

CMSize CM_FIXEDARGS CMGetValueDataOffset(CMValue value, CMCount offset, short *errVal)
{
  TOCValueHdrPtr theValueHdr;
  TOCValuePtr    theValue;
  ContainerPtr   container;
...
/*  if (cmCountListCells(&theValueHdr->valueList) > 1)  only one segment */
/*  	return(0xFFFFFFFF);   */
/* */
  theValue = cmGetStartingValue(theValueHdr, offset, &offset); /* get start seg/offset  */
  if (theValue == NULL)
  {
  	*errVal = (short)0xFFFF;
  	return (zero);                 /* offset out of range              */
  }

  omfsAddInt64toInt64(theValue->value.notImm.value, &offset);
  return (offset);    /* return file offset */
}
...
void CM_FIXEDARGS CMWriteValueData(CMValue value, CMPtr buffer, CMCount offset, CMSize32 size)
{
  TOCValueHdrPtr theValueHdr;
  ...
  unsigned char  *p;
  char           offsetStr[15];
  CMSize32		  len, remaining, amountWritten, valueSize32;
  CMCount		nextFree, endOffset;
  CMSize		valueSize, lenTmp;
  ...
  if ((theValueHdr->valueFlags & ValueProtected) != 0){ /* can't write if protected!    */
    ERROR1(container,CM_err_WriteIllegal2, CONTAINERNAME);
    return;
  }

  if ((theValueHdr->valueFlags & ValueGlobal) != 0) {   /* can't write to global names  */
    ERROR1(container,CM_err_CantWriteGlbl, CONTAINERNAME);
    return;
  }
  ...
  if (cmIsEmptyList(&theValueHdr->valueList)) {     /* if we have a empty value list... */
    if (omfsInt64Greater(offset, zero)) {              /* ...create initial value          */
      ERROR2(container,CM_err_Offset2Big, cml64tostr(offset, 1, false, offsetStr), CONTAINERNAME);
      return;
    }

    if (size <= sizeof(CM_ULONG)) {                 /* we can make value immediate...   */
      longVal = 0;
      memcpy(&longVal, buffer, size);
      (void)cmSetImmValueBytes(container, &valueBytes, Value_Imm_Chars, longVal, size);
      ...
      /* align new values according to container constraints */
		...
		omfsDivideInt64byInt32(nextFree, container->valueAlignment, NULL, &pastBoundary);
		if ((container->valueAlignment != 1) && (pastBoundary != 0))
	       ...
	       unsigned char  zeroBuf[16];
	       CMSize32		 i, overBytes;
	       unsigned char* zeroPtr = NULL;
			    ...
			    fillBytes = container->valueAlignment - overBytes;
	       		if ( fillBytes <= 16 )
	       			...
	       			zeroPtr = (unsigned char *)CMmalloc( container, fillBytes );

				for (i = 0; i< fillBytes; i++)
					zeroPtr[i] = 0;

	       if (CMfwrite(container, (void *)zeroPtr, 1, fillBytes) != fillBytes) {
	            ERROR1(container,CM_err_BadWrite, CONTAINERNAME);
	            if (fillBytes > 16) CMfree(container, zeroPtr);
	            return;
	       }
	       if (fillBytes > 16) CMfree(container, zeroPtr);
      }
      ...
      if (cmWriteData(theValueHdr, (unsigned char *)buffer, (unsigned int )size) != size)
        ERROR1(container,CM_err_BadWrite, CONTAINERNAME);
    }
  ...
  if (omfsInt64NotEqual(valueSize, offset)) {                        /* here we must be overwriting...   */
#if USE_UPDATE_MODE
    if (TouchIt(container, theValueHdr->container)){/* if recording updates...          */
      omfsCvtUInt32toInt64(size, &size64);
      CMDeleteValueData(value, offset, size64);       /* ...do overwrites this way!       */
    ...
    p         = (unsigned char *)buffer;            /* p points to next byte to write   */
    remaining = (CMSize32)size;                /* remaining bytes to write         */
    ...
    while (theValue && remaining) {                 /* overwrite existing value data    */
      ...
      offset = zero;                                   /* full segments from now on        */
    ...
    size = remaining;                       /* prepare to write rest at end     */
  ...
  if (theValue->flags & kCMImmediate) {             /* if current value immediate...    */
	omfsTruncInt64toUInt32(valueSize, &valueSize32);
    if (valueSize32 + size <= sizeof(CM_ULONG)) {     /* cram new data if we can...       */
      if (size > 0) {                               /* there must be some data to write */
        memcpy((char *)&theValue->value.imm.value + sizeof(CM_ULONG)+valueSize32, (char *)buffer, (size_t)size);
        theValue->value.notImm.valueLen = valueSize;
        omfsAddInt32toInt64(size, &theValue->value.notImm.valueLen);
        omfsAddInt32toInt64(size, &theValueHdr->size);
#if USE_UPDATE_MODE
  ...
  nextFree = CMgetContainerSize(container);
  endOffset = theValue->value.notImm.value;
  omfsAddInt64toInt64(theValue->value.notImm.valueLen, &endOffset);
  if (omfsInt64Equal(endOffset, nextFree) &&
      theValue->container == container) {           /* remember, must be SAME container!*/
    if (size > 0) {                                 /* there must be some data to write */
      CMfseek(container, zero, kCMSeekEnd);            /* make sure of container position  */
      if (CMfwrite(container, buffer, sizeof(unsigned char), size) != size) {
        ERROR1(container,CM_err_BadWrite, CONTAINERNAME);
        return;
      }
      container->physicalEOF = nextFree;
      omfsAddInt32toInt64(size, &container->physicalEOF); /* update next free container byte  */
      SetLogicalEOF(container->physicalEOF);        /* logical EOF == physical EOF      */
      omfsAddInt32toInt64(size, &theValue->value.notImm.valueLen); /* update total size                */
      omfsAddInt32toInt64(size, &theValueHdr->size);  /* keep size in valueHdr in sync    */
#if USE_UPDATE_MODE
  ...
  (void)cmSetImmValueBytes(container, &valueBytes, Value_NotImm, nextFree, size);
...
void CM_FIXEDARGS CMDefineValueData(CMValue value, CMCount offset, CMSize size)
{
  ...
  if ((container->useFlags & kCMConverting) == 0) { /* must be converting to a container*/
    ERROR1(container,CM_err_NotConverting, CONTAINERNAME);
    return;
  }
  ...
  endOffset = offset;
  omfsAddInt64toInt64(size, &endOffset);
  if (omfsInt64Greater(offset, container->maxValueOffset) ||
      omfsInt64Greater(endOffset, container->maxValueOffset)) {
    ERROR2(container,CM_err_BadDefineData, cml64tostr(offset, 1, false, offsetStr), CONTAINERNAME);
    return;
  }
  ...
  if (theValue->flags & kCMImmediate) {                   /* don't append to this!      */
    ERROR1(container,CM_err_BadDefineType, CONTAINERNAME);
    return;
  }
  ...
  endOffset = theValue->value.notImm.value;
  omfsAddInt64toInt64(theValue->value.notImm.valueLen, &endOffset);
  if (omfsInt64Equal(endOffset, offset)) {
    omfsAddInt64toInt64(size, &theValue->value.notImm.valueLen);    /* update total size */
    omfsAddInt64toInt64(size, &theValueHdr->size);                 /* keep size in valueHdr in sync    */
    return;
  }
...
void CM_FIXEDARGS CMInsertValueData(CMValue value, CMPtr buffer, CMCount offset, CMSize32 size)
{
  TOCValueHdrPtr theValueHdr;
  TOCValuePtr    theValue, insBeforeValue;
  ContainerPtr   container;
  unsigned char  *p, *q, *ins;
  CMSize		  valueSize, insSize, segOffset, insOffset, offset1, actualSize, largeSize;
  char           offsetStr[15];
  omfInt64		zero;
  CMSize32		valueSize32, offset32;

  omfsCvtUInt32toInt64(0, &zero);
  omfsCvtUInt32toInt64(size, &largeSize);
  ExitIfBadValue(value, CM_NOVALUE);                /* validate value                   */
  ...
  if (IsDynamicValue(value)) {                      /* process dynamic value...         */
    ...
    if (IsDynamicValue(value)) {
      ...
      AllowCMGetBaseValue(container);
      CMDynInsertValueData(value, buffer, offset, largeSize);
      DisAllowCMGetBaseValue(container);
  ...
  if (omfsInt64Equal(valueSize, offset)) {                        /* inserting to end of current value*/
  ...
  if (theValue->flags & kCMImmediate) {             /* if current value immediate...    */
    /* !!! */omfsTruncInt64toUInt32(valueSize, &valueSize32);
    if (valueSize32 + size <= sizeof(CM_ULONG)) {     /* insert new data if we can...     */
      insSize = largeSize;
      p = (unsigned char *)&theValue->value.imm.value + valueSize32;	/* !!! Check this out! */
      q = p + size;
      omfsTruncInt64toUInt32(offset, &offset32);
      ins = (unsigned char *)&theValue->value.imm.value + offset32;	/* !!! Check this out! */
      while (--p >= ins) *--q = *p;
      q = (unsigned char *)buffer;
      while (size--) *++p = *q++;
      theValue->value.notImm.valueLen = insSize;
      omfsAddInt32toInt64(valueSize32, &theValue->value.notImm.valueLen);
      omfsAddInt64toInt64(insSize, &theValueHdr->size);
#if USE_UPDATE_MODE
      cmTouchImmediateValue(theValueHdr);           /* touch for updating if necessary  */
#endif
      return;                                       /* that's all we need to do!        */
  ...
  insOffset = cmGetFreeListEntry(container, largeSize, true, &actualSize);
  if (omfsInt64NotEqual(actualSize, zero))          /* if we got some to reuse...       */
    ...
    CMfseek(container, zero, kCMSeekEnd);
  }
  ...
  offset1 = insOffset;
  omfsAddInt32toInt64(size, &offset1);            /* update logical OR physical EOF   */
  if (omfsInt64Equal(actualSize, zero))
  ...
  cmInsertNewSegment(theValueHdr, insBeforeValue, segOffset, insOffset, largeSize);
...
void CM_FIXEDARGS CMDeleteValueData(CMValue value, CMCount offset, CMSize size)
{
  ...
  ContainerPtr   container;
  unsigned char  *p, *q;
  CMSize		  valueSize, delSize;
  CMCount		startOffset, endOffset;
  CMCount32		startOff32, endOff32, valueSize32;
  omfInt64		zero, one;
  ...
  valueSize = CMGetValueSize((CMValue)theValueHdr); /* get current size of value        */
  if (omfsInt64GreaterEqual(offset, valueSize)) return;  /* if deleting from end, just exit  */

  /* Ok, damn it, we got to do some work and delete something.  Let's get the starting  */
  ...
  omfsAddInt64toInt64(size, &endOffset);
  omfsSubInt64fromInt64(one, &endOffset);          /* end offset limited to end of data*/
  if (omfsInt64GreaterEqual(endOffset, valueSize))
  {
    endOffset = valueSize;
    omfsSubInt64fromInt64(one, &valueSize);
  }
  ...
  if (theValue->flags & kCMImmediate) {             /* if immediate...                  */
    p  = (unsigned char *)&theValue->value.imm.value; /* !!!! */ /* ...do a simple byte delete       */
    /* !!! */omfsTruncInt64toUInt32(startOffset, &startOff32);
    /* !!! */omfsTruncInt64toUInt32(endOffset, &endOff32);
    /* !!! */omfsTruncInt64toUInt32(valueSize, &valueSize32);
    q  = p + startOff32;
    p += endOff32 + 1;
    omfsCvtUInt32toInt64(endOff32 - startOff32 + 1, &delSize);
    omfsSubInt64fromInt64(delSize, &theValueHdr->size);
    theValue->value.notImm.valueLen = theValueHdr->size;
    while (++endOff32 < valueSize32)
    {
    	*q++ = *p++;
    }
#if USE_UPDATE_MODE
  ...
  /* See comments in cmDeleteSegmentData() (in    Values.c    ).                        */
...
void CM_FIXEDARGS CMMoveValue(CMValue value, CMObject object, CMProperty property)
{
  ...
  if ((container->updatingContainer->useFlags & kCMWriting) == 0) {
    ERROR1(container,CM_err_MoveIllegal, CONTAINERNAME);
    return;
  }
  ...
  if (IsDynamicValue(theFromValueHdr)) {                /* if we have dynamic value...  */
    ...
    if (theRealValueHdr->dynValueData.dynValue == NULL || /* do the validation...       */
      ...
      ERROR2(container,CM_err_BadRealValue, "CMMoveValue", CONTAINERNAME); /* ...oops!            */
      return;
    }
...
void CM_FIXEDARGS CMSetValueType(CMValue value, CMType type)
{
  ...
  if (theValueHdr0 != NULL && theValueHdr != theValueHdr0) {
    ERROR2(container,CM_err_DupType, cmGetGlobalTypeName(container, typeID), CONTAINERNAME);
    return;
  }
...
void CM_FIXEDARGS CMSetValueGeneration(CMValue value, CMGeneration generation)
{
  ...
  if (generation < 1) {                             /* validate the generation nbr      */
    ERROR3(container,CM_err_BadGenNbr, cmltostr(generation, 1, false, genStr), "CMSetValueGeneration", CONTAINERNAME);
    return;
  }
...
void CM_FIXEDARGS CMDeleteValue(CMValue value)
{
  ...
  if ((container->useFlags & kCMWriting) == 0) {
    ERROR2(container,CM_err_DeleteIllegal, "value", CONTAINERNAME);
    return;
  }

  if ((theValueHdr->valueFlags & (ValueProtected | ValueUndeletable)) != 0) {
    ERROR1(container,CM_err_CantDelete3, CONTAINERNAME);
    return;
  }

  if (theValueHdr->useCount > 1) {                      /* there are other users!       */
    ERROR1(container,CM_err_CantDelete4, CONTAINERNAME);          /* nice try though!             */
    return;
  }
  ...
  if (IsDynamicValue(theValueHdr)) {                    /* if we have dynamic value...  */
    ...
    if (theRealValueHdr->dynValueData.dynValue == NULL || /* do the validation...       */
      ...
      ERROR2(container,CM_err_BadRealValue, "CMDeleteValue", CONTAINERNAME); /* ...oops!          */
      return;
    }
...
void CM_FIXEDARGS CMReleaseValue(CMValue value)
{
  ...
  if (theValueHdr == dynamicBaseValueHdr) {         /* if attempted release on base...  */
    ERROR1(container,CM_err_BaseRelAttempted, CONTAINERNAME); /* ...yell                          */
    return;
  }
  ...
  if (theValueHdr->useCount == 0) {
    ERROR1(container,CM_err_AlreadyReleased1, CONTAINERNAME);
    return;
  }
  ...
  do {                                              /* loop up through all the layers...*/
    ...
    if ((theValueHdr->valueFlags & ValueOffPropChain) != 0) {   /* if upper layer...    */
      if (cmKeepDeletedRefNums(container->toc)) {               /* ...keep refNums ?    */
        ...
        CMfree(container, theValueHdr);                                    /* ...free it           */
    ...
    CMfree(container, extensions);                             /* free the extensions              */
