...
static CMSize seek_Handler(CMRefCon refCon, CM_LONG posOff, CMSeekMode mode)
{
  ...
  result = fseek(p->f, (int )posOff, seekMode); /* seek...                              */

  if (p->tracing) {                             /* tracing...                           */
    int  newPosition = ftell(p->f);
  ...
  p->lastSeekOffset = (int )posOff;             /* and also the specified offset        */
                               ...
                               CMSize tocOffset, CMSize tocSize)
{
  ...
  if (labelSize != sizeof(ContainerLabelFmt))   /* must have written it all!            */
    CMError(p->sessionData, "Incorrect byte length written while writing label to container \"^0\"!", p->pathname);
}
