...
struct TOC {                            /* Layout for a TOC "control block":            */
  ...
  short          nbrOfIndices;          /*    nbr of index table levels (table depth)   */
  unsigned int   tableSize;             /*    size of an index table                    */
  Boolean        powerOfTwo;            /*    true ==> can access index by shift/mask   */
  short          indexShift;            /*    shift value to access an index(powerOfTwo)*/
  unsigned int   indexMask;             /*    mask value to access an index (powerOfTwo)*/
  ...
  short          useCount;              /*    number of distinct users of this TOC      */
  Boolean        refNumHandling;        /*    true ==> keep deleted object/value refNums*/
...
void *cmCreateTOC(unsigned int  tableSize, const struct Container *container)
{
  short         bit, bitsPerLong;
  ...
  t = (TOCPtr)CMmalloc(container, sizeof(TOC));                /* a "control block" is born!       */
  ...
  while (maxValue /= tableSize)                     /* see how int  it takes to reduce  */
  ...
  /* Determine the bits size of a "unsigned int ".  This is done by counting the bits   */
...
static unsigned short *CM_NEAR getIndices(const TOCPtr t, CMObjectID id, register unsigned short *indices)
{
  short i = 0;
  ...
  if (t->powerOfTwo)                                /* if we can shift and mask...      */
    do {                                            /* ...what are you waiting for?     */
      *indices++ = (unsigned short)((unsigned int )id & t->indexMask);
      id >>= t->indexShift;
      ++i;
    } while (id);
  else                                              /* if we can't do shifts...         */
    do {                                            /* ...bummer!                       */
      *indices++ = (unsigned short)((unsigned int )id % t->tableSize);
      id /= t->tableSize;
      ++i;
    } while (id);
...
TOCObjectPtr cmCreateObject(const void *toc, const CMObjectID objectID,
                            unsigned short objectFlags, Boolean *dup)
{
  register TOCPtr  t = (TOCPtr)toc;
  register Indices *t1, *t2;
  unsigned int     i, tableSize = t->tableSize;
  unsigned short   indices[32], *index, x;
  short            n = t->nbrOfIndices;
  TOCObjectPtr     p;
  ...
  if (t->toc == NULL) {                                   /* if 1st time...             */
    t->toc = (Indices)CMmalloc(container, tableSize * sizeof(void *)); /* ...create root           */
  ...
  while (n-- > 1) {                                       /* use all the indicies...    */
    ...
    if (t2 == NULL) {                                     /* if we're missing a tbl...  */
      t2 = (Indices *)CMmalloc(container, tableSize * sizeof(void *)); /* ...create it             */
  ...
  else if ((p = (TOCObjectPtr)CMmalloc(container, sizeof(TOCObject))) != NULL){/* create new object*/
                      ...
                      void (*freeAction)(TOCObjectPtr object, CMRefCon refCon))
{
  ...
  CMfree(container, object);                                   /* free the object                  */
                     ...
                     void (*deleteAction)(TOCObjectPtr object, CMRefCon refCon))
{
  ...
  if (((TOCPtr)toc)->refNumHandling) {              /* if keep deleted object refNums...*/
    ...
    container = ((TOCPtr)toc)->container;           /* ...needed for CMfree(container, )           */
    CMfree(container, object);                                 /* ...free the object space         */
  }
...
static void CM_NEAR scanIndexTable(register const TOCPtr toc, unsigned short *index,
                                   CMObjectID endingID, const Indices *t,
                                   const short indexLevel, CMRefCon refCon,
  ...
  if (indexLevel < toc->nbrOfIndices)                         /* if we have index tbl...*/
    for (i = *index, *index-- = 0; i < toc->tableSize; i++) { /* ...look at each entry  */
      if (t[i])                                               /* ...if non-NULL entry...*/
        scanIndexTable(toc, index, endingID, (Indices *)t[i], (short)(indexLevel+1), refCon, action);
    }
...
{
  short          n;
  unsigned short *index, indices[32];
  ContainerPtr   container = ((TOCPtr)toc)->container;
  ...
  if (((TOCPtr)toc)->toc) {                         /* scanIndexTable() does everything */
    if (startingID == ALLOBJECTS) {                 /* if frocessing ALL objects...     */
      ...
      index = indices;		                        /*    the 0 index will cause all    */

      while (n-- > 0)
		{
		*index = 0;					                /*    index table entrys to be used */
		index++;
		}
	index--;
                                   ...
                                   void (*deleteAction)(TOCObjectPtr object, CMRefCon refCon))
{
  ...
  if (indexLevel < toc->nbrOfIndices)               /* if we have index table...        */
    for (i = 0; i < toc->tableSize; i++) {          /* ...look at each entry...         */
      if (t[i])                                     /* ...if non-NULL entry...          */
        freeAllObjects(toc, (Indices *)t[i], (short)(indexLevel+1), refCon, deleteAction);
    }
  else                                              /* if last index table...           */
    for (i = 0; i < toc->tableSize; i++)            /* ...we still look at each entry...*/
      if ((object = (TOCObjectPtr)(t[i])) != NULL) {/* ...if non-NULL entry...          */
        ...
        CMfree(container, object);                             /* ...free the object itself        */
      }

  CMfree(container, t);                                        /* free the index table too         */
}
...
void cmDestroyTOC(void **toc, CMRefCon refCon,
                  void (*deleteAction)(TOCObjectPtr object, CMRefCon refCon))
{
  ...
  if (((TOCPtr)*toc)->toc)                          /* bye, bye TOC...                  */
    freeAllObjects((TOCPtr)*toc, (Indices *)((TOCPtr)*toc)->toc, 1, refCon, deleteAction);
...
#if USE_UPDATE_MODE
    cmDeleteTouchedList(theObject);                 /* free touched list (if any)       */
#endif
    CMfree(container, theObject);                   /* that's end of this object        */
    theObject = nextObject;
  }

  CMfree(container, *toc);                          /* bye, bye TOC control block       */
                          ...
                          TOCObjectPtr afterThisObject)
{
  ...
  /* Flag the object so we never try to do this crap again!  See start of code above.   */
...
TOCObjectPtr cmGetMasterListHead(const void *toc, unsigned short objectKindFlag)
{
...
void cmChangeRefNumHandling(const void *toc, Boolean setting)
{
...
Boolean cmKeepDeletedRefNums(const void *toc)
{
