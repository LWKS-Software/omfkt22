...
struct GetGNameComArea {          /* CMGetGlobalName() object walking comm area layout: */
  ...
  unsigned short nbrOfGlobalNames;/*    count of nbr of global names found in object    */
};
 ...
 *----------------------------------------------------------------------------*
 ...
 This routine returns the refNum for the next object in the container following currObject.
 If currObject is NULL, the refNum for the first object is returned.  If currObject is not
 NULL, the object with the next highest ID is returned.  NULL is returned if there are no
 more objects following currObject.
...
CMObject CM_FIXEDARGS CMGetNextObject(CMContainer targetContainer, CMObject currObject)
{
  ...
  if (currObject == NULL) {                         /* NULL ==> use head of list        */
  ...
  if (container->targetContainer != ((TOCObjectPtr)currObject)->container->targetContainer) {
    ERROR3(container,CM_err_BadContainer, "object", CONTAINERNAMEx(((TOCObjectPtr)currObject)->container), CONTAINERNAMEx(container));
    return (NULL);
  }
 ...
 *--------------------------------------------------------------------------*
 ...
 currProperty is NULL, the refNum for the first property for the object is returned.  If 
 it is not NULL, the next property refNum following currProperty is returned.
...
CMProperty CM_FIXEDARGS CMGetNextObjectProperty(CMObject theObject, CMProperty currProperty)
{
  ...
  /* first property on theObject.                                                       */
  
  if (currProperty == NULL) {                       /* NULL ==> use head of list        */
#ifdef VIRTUAL_BENTO_OBJECTS
  {
    if(cmLoadUpStubObject((TOCObjectPtr)theObject) == NULL)
    	return(NULL);
  }
#endif
  ...
  /* Here we ant to find the next property for the object.  Since currProperty is not   */
  /* NULL, it points to an object because it's a refNum.  Remember it is not a          */
  ...
  /* simply get the next on on the list to get the next property ID.                    */
  ...
  if (theProperty) {                                /* if we found ID on the list...    */
    ...
    if (theProperty) {                              /* if there's a next property...    */
      theNextObject = cmFindObject(container->toc, theProperty->propertyID); /* get obj */
      if (theNextObject) ++theNextObject->useCount; /* bump its use count               */
      return ((CMProperty)theNextObject);           /* give caller the property object  */
    }
  }
 ...
 *-----------------------------------------------------------------------------------*
 ...
 This routine returns the refNum for the next object in the container that has the given
 property.  If currObject is NULL, the search starts with the first object in the 
 container.  If it is not NULL the search starts with the object immediately following
 currObject.  If there is no next object with the given property, NULL is returned. 
...
CMGlobalName CM_FIXEDARGS CMGetGlobalName(CMObject theObject)
{
  ...
  if (gNameComArea.nbrOfGlobalNames > 1) {        /* must have exactly 1 global name    */
    ERROR2(container,CM_err_AmbiguousType, "CMGetGlobalName", CONTAINERNAME);
    return (NULL);
  }
...
void CM_FIXEDARGS CMDeleteObject(CMObject theObject)
{
  ...
  if ((container->useFlags & kCMWriting) == 0) {      /* make sure opened for writing   */
    ERROR2(container,CM_err_DeleteIllegal, "object", CONTAINERNAME);
    return;
  }

  if (((TOCObjectPtr)theObject)->useCount > 1) {      /* there are other users!         */
    ERROR1(container,CM_err_CantDelete6, CONTAINERNAME);
    return;
  }
  ...
  if (((TOCObjectPtr)theObject)->objectFlags & ProtectedObject)
    ERROR1(container,CM_err_CantDelete1, CONTAINERNAME);
  else if (((TOCObjectPtr)theObject)->objectFlags & DynamicValuesObject)
    ERROR1(container,CM_err_CantDelete5, CONTAINERNAME);
  else {
...
void CM_FIXEDARGS CMDeleteObjectProperty(CMObject theObject, CMProperty theProperty)
{
  ...
  if ((container->useFlags & kCMWriting) == 0) {
    ERROR2(container,CM_err_DeleteIllegal, "object's property", CONTAINERNAME);
    return;
  }
  ...
  while (theValueHdr) {
    if ((theValueHdr->valueFlags & ValueProtected) != 0){ /* can't delete if protected! */
      ERROR1(container,CM_err_CantDelete2, CONTAINERNAME);
      return;
    }
...
void CM_FIXEDARGS CMReleaseObject(CMObject theObject)
{
  ...
  if (((TOCObjectPtr)theObject)->useCount == 0) { /* error if already fully released    */
    container = ((TOCObjectPtr)theObject)->container;
   ERROR1(container,CM_err_AlreadyReleased2, CONTAINERNAME);
  } else
...
#ifdef VIRTUAL_BENTO_OBJECTS
/*------------------------------------------------------------------------------*
 | CMPurgeObjectVM - Flush the property & value data if it has not been touched |
 *------------------------------------------------------------------------------*
...
void CM_FIXEDARGS CMPurgeObjectVM(CMObject theObject)
{
  TOCObjectPtr	tocObj = (TOCObjectPtr)theObject;
  
  ExitIfBadObject(theObject, CM_NOVALUE);         /* validate theObject                 */
  if(!tocObj->container->objectsMayBeVirtual)
  	return;
  
  /* Only flush the object if it is loaded and not "touched" */
  if((tocObj->objectFlags & (ObjectIsNotLoaded | TouchedObject)) == 0)
  {
	cmFreeProperties(tocObj, tocObj->container);
	(void)cmInitList(&(tocObj->propertyList));
	/* !!! Now clean up whatever lists the object keep internally referencing the properties */
	tocObj->objectFlags |= ObjectIsNotLoaded;
  }
}
#endif
