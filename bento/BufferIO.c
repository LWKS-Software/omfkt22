...
struct IOControl {                        /* Layout of an I/O control block:            */
  ...
  unsigned int    maxBufSize;             /*    max size of the buffer                  */
  int             bufferPosition;         /*    0-rel. next byte to use in updateBuffer */
  Boolean		  bufinit;					/* JEFF: used at buffer creation */
  CMCount		   bufferReadOffset;       /*    offset to next buffer to read in value  */
  CMSize		   dataSize;               /*    total size of data to be read           */
  ...
  char            dataBuffer[1];          /*    the START of the data I/O buffer        */
};
...
void *cmUseIOBuffer(ContainerPtr container, int  maxBufferSize, jmp_buf *ioEnv)
{
  IOControlPtr io;

  if ((io = (IOControlPtr)CMmalloc(container, sizeof(IOControl) + maxBufferSize)) == NULL) {
    ERROR1(container,CM_err_NoDataBuffer, CONTAINERNAME);
    return (NULL);
  }

  io->container      = container->updatingContainer;/* remember the updatingContainer   */
  ...
  return ((void *)cmAppendListCell(&io->container->activeIOBuffers, io));
}
...
void cmReleaseIOBuffer(void *ioBuffer)
{
  ...
  if (ioBuffer != NULL) {
    container = ((IOControlPtr)ioBuffer)->container;
    CMfree(container, cmDeleteListCell(&container->activeIOBuffers, ioBuffer));/*unlink/free buffer*/
  }
}
...
void cmNewBufferedOutputData(void *ioBuffer, TOCValueHdrPtr theValueHdr)
{
  ...
  io->ioPos          = (theValueHdr != NULL) ? zero  /* remember where we are            */
                                             : CMftell(io->container);
}
...
void cmWriteBufferedData(void *ioBuffer, CMSize32 size, CM_ULONG data4)
{
  IOControlPtr  io        = (IOControlPtr)ioBuffer;
  ...
  if (size == 1) {                              /* 1 ==> pointer to CM_UCHAR            */
    data1 = (CM_UCHAR)data4;
    d = (CMPrivateData)&data1;
  } else if (size == 2) {                       /* 2 ==> pointer to CM_USHORT           */
  ...
  /* bytes less than the true maximum buffer size.  This means that the next free byte  */
  /* could be passed this imposed limit.  At that point we write the buffer as (value)  */
  /* data and reset the offset.  By doing this we never have to worry about not being   */
  /* able to write 2 or 4 bytes to the buffer.  We also document in the handler that we */
  /* always guarantee enough space in the buffer.  The artifical limit does that.  4 is */
  /* enough of a pad (actually we use sizeof(CM_ULONG)) to allow the max write of 4     */
  /* bytes.                                                                             */

  io->bufferPosition += size;                 /* update offset to the next free byte    */

  /* If the buffer needs flushing, now's the time to do it...                           */

  if ((omfUInt32)io->bufferPosition >= io->maxBufSize - sizeof(CM_ULONG))  /* flush if full        */
    cmFlushOutputBuffer(ioBuffer);
}
  ...
  if (io->bufferPosition > 0) {                       /* flush if somthing to flush     */
    if (io->theValueHdr != NULL) {                    /* high or low level I/O ?        */
      CMWriteValueData((CMValue)io->theValueHdr, (CMPtr)io->dataBuffer,
                       io->theValueHdr->size, io->bufferPosition);
      omfsAddInt32toInt64(io->bufferPosition, &io->ioPos);  /* update output position         */
    } else if (CMfwrite(container, io->dataBuffer, sizeof(unsigned char), io->bufferPosition) == (omfUInt32)io->bufferPosition) {
      io->ioPos = CMftell(container);  /* get "true" container position  */
      if (omfsInt64Greater(io->ioPos, container->physicalEOF))  /* make sure of EOF settings...   */
                            ...
                            unsigned int  dataSize)
{
  ...
  io->bufinit = TRUE;
  omfsCvtUInt32toInt64(0, &io->bufferReadOffset); /* 1st refNum offset to will be 0   */
  io->theValueHdr      = theValueHdr;               /* remember refNum (if any)         */
  omfsCvtUInt32toInt64(dataSize, &io->dataSize);     /* and max amount to read           */
  io->ioPos            = (theValueHdr != NULL) ? zero/* remember where we are            */
                                             :  CMftell(io->container);
}
...
CM_ULONG cmReadBufferedData(void *ioBuffer, CMSize32 size)
{
  IOControlPtr  io        = (IOControlPtr)ioBuffer;
  ContainerPtr  container = io->container;
  CM_UCHAR      data1;
  ...
  CM_ULONG      data4;
  CMPrivateData d;
  CMSize32        bufSize, amountRead;
  unsigned int  nextPos;
  unsigned int  remain32;
  CMSize		remaining;
  ...
  nextPos = io->bufferPosition + size;            /* next byte to read after this one   */

  if (nextPos > io->maxBufSize) {                 /* reload if all bytes not available  */
    if(io->bufinit)
    	io->bufinit = FALSE;
    else
    	omfsAddInt32toInt64(io->bufferPosition, &io->bufferReadOffset);   /* next value offset                  */
    remaining = io->dataSize;
    omfsSubInt64fromInt64(io->bufferReadOffset, &remaining);  /* data left to read              */
    if(omfsTruncInt64toUInt32(remaining, &remain32) != OM_ERR_NONE)
    	bufSize = io->maxBufSize;
    else
   		 bufSize   = (remain32 > io->maxBufSize) ? io->maxBufSize : remain32;

    if (io->theValueHdr != NULL)
      amountRead = CMReadValueData((CMValue)io->theValueHdr, (CMPtr)io->dataBuffer,
                                   io->bufferReadOffset, bufSize);
    else {
      omfsAddInt32toInt64(io->bufferPosition, &io->ioPos);  /* we will reread unprocessed stuff   */
      CMfseek(container, io->ioPos, kCMSeekSet);
      amountRead = CMfread(container, io->dataBuffer, sizeof(unsigned char), bufSize);
    }
    ...
    omfsAddInt32toInt64(amountRead, &io->ioPos);  /* update input position              */
    io->bufferPosition = 0;                       /* reset the buffer offset            */
    nextPos = (int )size;                         /* reset the next offset after that   */
  }
...
CMCount cmBufferedIOftell(void *ioBuffer)
{
