...
union TOCValueBytes {                 /* Layout of value/length fields:                 */
  struct {                            /*    value if not immediate(not explicitly here):*/
    CMCount value;                   /*        offset to value if not immediate        */
    CMSize valueLen;                /*        value length if not immediate           */
  } notImm;

  struct {                            /*    value for a global name                     */
    CMCount          offset;         /*        offset to value in container            */
    struct GlobalName *globalNameSymbol;/*      ptr value for a global name (in memory) */
  } globalName;

  union {
    CM_UCHAR  pad[2*sizeof(CM_ULONG)];/*value if immediate unsigned char(s)     */
    CMCount   value;                /*        value if immediate unsigned int         */
  } imm;
};
...
struct TOCValue {                     /* Layout of a TOC type's value:                  */
  ...
  CMCount		      logicalOffset;   /*    original (unedited) logical offset          */
};
...
enum ConstValueType {                 /* Data types used to copy data into TOCValue's:  */
  ...
  Value_Imm_Long,                     /*    immediate, int   ==> ulongValue             */
...
struct TOCValueHdr {                  /* Layout of a TOC property type:                 */
  ...
  ContainerPtr        container;      /*    ptr to "owning" container control block     */
  CMSize       size;           /*    total current size of the value data        */
  CMSize       logicalSize;    /*    original (unupdated) size of the value data */
  unsigned short      valueFlags;     /*    flags indicating stuff about the value      */
  CMGeneration        generation;     /*    generation number                           */
  unsigned int        useCount;       /*    count of nbr of times "used"                */
...
CM_EXPORT unsigned short cmSetTOCValueHdrFlags(TOCValueHdrPtr theValueHdr,
...
CM_EXPORT TOCValueBytesPtr cmSetValueBytes(const ContainerPtr container,
                                           TOCValueBytesPtr theValueBytes,
                                           ConstValueType dataType, CMCount data,
                                           CMSize dataLength);

/* expand to CMCount & CMSize versions & call cmSetValueBytes */
CM_EXPORT TOCValueBytesPtr cmSetImmValueBytes(const ContainerPtr container,
                                              TOCValueBytesPtr theValueBytes,
                                              ConstValueType dataType, CMCount data,
                                              CMSize dataLength);
...
CM_EXPORT TOCValuePtr cmCreateValueSegment(TOCValueHdrPtr theValueHdr, TOCValueBytesPtr value,
                                           const unsigned short flags);
  /*
...
CM_EXPORT TOCValuePtr cmAppendValue(TOCValueHdrPtr theValueHdr, TOCValueBytesPtr value,
                                    const unsigned short flags);
  /*
...
CM_EXPORT TOCObjectPtr cmDefineObject(ContainerPtr container, unsigned int  objectID,
...
typedef enum { LoadAsStub, LoadAllFields } omfObjectLoadType_t;

CM_EXPORT TOCObjectPtr cmIntDefineObject(ContainerPtr container, unsigned int  objectID,
                                         const unsigned int  propertyID, const unsigned int  typeID,
                                         TOCValueBytesPtr value, const unsigned int  generation,
                                         const unsigned short flags, const unsigned short objectFlags,
...
CM_EXPORT void cmMarkValueDeleted(ContainerPtr container, TOCValueHdrPtr theValueHdr,
                                  Boolean deleteHdr);
  /*
...
CM_EXPORT void cmDeleteProperty(ContainerPtr container, TOCPropertyPtr theProperty);
  /*
...
CM_EXPORT void cmMarkObjectDeleted(ContainerPtr container, TOCObjectPtr theObject);
  /*
...
CM_EXPORT void cmFreeTOC(ContainerPtr container, void **toc);
  /*
...
CM_EXPORT TOCPropertyPtr cmGetObjectProperty(TOCObjectPtr theObject, unsigned int  propertyID);
  /*
...
CM_EXPORT TOCValueHdrPtr cmGetPropertyType(TOCPropertyPtr theProperty, unsigned int  typeID);
  /*
...
CM_EXPORT CMSize cmGet1ValueSize(TOCValuePtr theValue);
  /*
...
CM_EXPORT Boolean cmValidateCMValue(CMValue value);
CM_EXPORT Boolean cmValidateCMObjects(CMObject object, unsigned short objectFlags);
  /*
...
CM_EXPORT int cmWalkThroughEntireTOC(ContainerPtr container, void *toc,
...
CM_EXPORT int cmWalkObject(ContainerPtr container, TOCObjectPtr theObject, CMRefCon refCon,
                           TOCWalkReturns (*objectAction)(ContainerPtr container, TOCObjectPtr theObject, CMRefCon refCon),
                           TOCWalkReturns (*propertyAction)(ContainerPtr container, TOCPropertyPtr theProperty, CMRefCon refCon),
                           TOCWalkReturns (*valueHdrAction)(ContainerPtr container, TOCValueHdrPtr theValueHdr, CMRefCon refCon),
                           TOCWalkReturns (*valueAction)(ContainerPtr container, TOCValuePtr theValue, CMRefCon refCon));
...
CM_EXPORT void cmFreeProperties(TOCObjectPtr theObject, CMRefCon refCon);
