...
static int  local_mem_ftell(omfStreamDescriptorPtr_t f);

static int  local_mem_feof(omfStreamDescriptorPtr_t);

static int local_mem_fflush(omfStreamDescriptorPtr_t);

static int local_mem_fclose(omfStreamDescriptorPtr_t);

static Boolean local_mem_fopen(omfStreamDescriptorPtr_t, char *);

static size_t local_mem_fread(void *, size_t, size_t, omfStreamDescriptorPtr_t);

static int local_mem_fseek(omfStreamDescriptorPtr_t, int , int);

static size_t local_mem_fwrite(const void *, size_t, size_t, omfStreamDescriptorPtr_t);
			                ...
			                omfHdl_t	file)
{
	...
	omfStreamDescriptorPtr_t pMem = (omfStreamDescriptorPtr_t)pathname;
...
static CMRefCon open_Handler(CMRefCon attributes, CMOpenMode mode)
{
	...
	switch (p->strm->streamType)
	{
	case kOmfStreamMemory:
		result = local_mem_fopen(p->strm, (char *) mode);
		break;

	case kOmfStreamFile:
		result = TRUE;
		p->strm->fPtr = fopen(p->strm->streamPtr, (char *) mode);	/* set file variable in
								 * refCon  */
		if (p->strm->fPtr == NULL)
		  {			/* oops, open didn't work!      */
			omfCMError(p->sessionData, "Cannot open \"^0\"!\n    ^1",
					   p->strm->streamPtr, strerror(errno));
			result = FALSE;
		  }
		else
			setvbuf(p->strm->fPtr, NULL, _IOFBF, 6000);	/* enlarge the I/O buffer       */
		break;
...
static CMSize32   flush_Handler(CMRefCon refCon)
{
	...
	{
	case kOmfStreamMemory:
		result = 0;	/* no flush for memory streams   */
		break;

	case kOmfStreamFile:
		result = fflush(p->strm->fPtr);	/* yup, you guessed it!         */
		break;
...
//		return (0);	/* ...there's no need for another seek  */
	...
	{
	case kOmfStreamMemory:
		result = local_mem_fseek(p->strm, (int ) pos32, seekMode);	/* seek...          */
		break;

	case kOmfStreamFile:
		result = fseek(p->strm->fPtr, (int ) pos32, seekMode);	/* seek...          */
		break;
...
static CMSize   tell_Handler(CMRefCon refCon)
{
	...
	{
	case kOmfStreamMemory:
		posOff = local_mem_ftell(p->strm);		/* get position       */
		break;

	case kOmfStreamFile:
		posOff = ftell(p->strm->fPtr);	/* get position       */
		break;
...
static CMSize32   uncachedRead(CMRefCon refCon, CMPtr buffer, CMSize32 elementSize, CMCount32 theCount)
{
	...
	{
	case kOmfStreamMemory:
		amountRead = local_mem_fread((char *) buffer, (size_t) elementSize, (size_t) theCount, p->strm);
		break;

	case kOmfStreamFile:
		amountRead = fread((char *) buffer, (size_t) elementSize, (size_t) theCount, p->strm->fPtr);
		break;
			                      ...
			                      CMCount32 theCount)
{
	...
	{			/* do write only if there's data  */
		...
		{
		case kOmfStreamMemory:
			amountWritten = local_mem_fwrite((char *) buffer, (size_t) elementSize, (size_t) theCount, p->strm);
			break;

		case kOmfStreamFile:
			amountWritten = fwrite((char *) buffer, (size_t) elementSize, (size_t) theCount, p->strm->fPtr);
			break;
...
static CMEofStatus eof_Handler(CMRefCon refCon)
{
	...
	{
	case kOmfStreamMemory:
		status = (int ) local_mem_feof(p->strm);
		break;

	case kOmfStreamFile:
		status = (int ) feof(p->strm->fPtr);
		break;
...
static CMSize   containerSize_Handler(CMRefCon refCon)
{
	...
	{			/* if size is not known...      */
		...
		{
		case kOmfStreamMemory:
			local_mem_fseek(p->strm, 0L, SEEK_END);	/* ...seek to the end of file   */
			p->fileSize = local_mem_ftell(p->strm);	/* ...size is where we are      */
			break;

		case kOmfStreamFile:
			fseek(p->strm->fPtr, 0L, SEEK_END);	/* ...seek to the end of file   */
			p->fileSize = ftell(p->strm->fPtr);	/* ...size is where we are      */
			break;
...
/*
 *
 *     LOW LEVEL HANDLER IMPLEMENTATIONS
 *
 *
 */
...
{
	assert(f != NULL);
	assert(f->streamType == kOmfStreamMemory);
	assert(f->magic == OMFSTREAM_GOOD_MAGIC);
	return(f->currentPos);

}
static int  local_mem_feof(omfStreamDescriptorPtr_t f)
{
	assert(f != NULL);
	assert(f->streamType == kOmfStreamMemory);
	assert(f->magic == OMFSTREAM_GOOD_MAGIC);
...
static Boolean local_mem_fopen(omfStreamDescriptorPtr_t f, char *mode)
{
	omfStreamStatus_t openStat;

	assert(f != NULL);
	assert(f->streamType == kOmfStreamMemory);

	if (strcmp(mode, "rb") == 0)
		openStat = kOmfStreamReadOnly;
	else if (strcmp(mode, "rb+") == 0)
		openStat = kOmfStreamAppend;
	else if (strcmp(mode, "wb+") == 0)
		openStat = kOmfStreamCreate;
	else
		return(FALSE); /* bad mode */

	f->openStatus = openStat;
	f->magic = OMFSTREAM_GOOD_MAGIC;

	switch (openStat) {
	case kOmfStreamReadOnly:
	case kOmfStreamAppend:
		if (f->streamPtr == NULL || f->streamSize == 0 || f->endOfData == 0)
			return(FALSE);

		f->currentPos = 0;
		break;

	case kOmfStreamCreate:
		if (f->initialBlock <= 0)
			return(FALSE);

		f->streamPtr = (char *)omfsMalloc(f->initialBlock);
		if (f->streamPtr == NULL)
			return(FALSE);

		f->streamSize = f->initialBlock;
		f->currentPos = 0;
		f->endOfData = 0;
		break;
...
static size_t local_mem_fread(void *buffer, size_t itemSize, size_t numItems, omfStreamDescriptorPtr_t f)
{
	...
	memcpy(buffer, (void *)&(f->streamPtr[f->currentPos]), numBytesToRead);
...
static int local_mem_fseek(omfStreamDescriptorPtr_t f, int  offset, int mode)
{
	int filePos = 0;
...
static int local_mem_fclose(omfStreamDescriptorPtr_t f)
{
	assert(f != NULL);
	assert(f->streamType == kOmfStreamMemory);
	assert(f->magic == OMFSTREAM_GOOD_MAGIC);

	f->magic = OMFSTREAM_BAD_MAGIC; /* prevent further operations */

	return(0);

}

static size_t local_mem_fwrite(const void *buffer, size_t itemSize, size_t numItems, omfStreamDescriptorPtr_t f)
{
	int  numBytesToWrite;
	int  itemsWritten;
	...
	if (numBytesToWrite > (f->streamSize - f->currentPos)) /* past end of memory */
	{
		char *tmpPtr = f->streamPtr;
		int  sizeToIncrease;
		...
		while ( ((f->streamSize + sizeToIncrease) - f->currentPos) < numBytesToWrite)
			sizeToIncrease += f->blockSize;

		f->streamPtr = (char *)omfsMalloc(f->streamSize + sizeToIncrease);
		if (f->streamPtr == NULL)
		{
			f->streamPtr = tmpPtr;
			return(0);
		}
		f->streamSize += sizeToIncrease;
		memcpy((void *)f->streamPtr, (void *)tmpPtr, f->endOfData); /* only copy data, not extra stuff */
		omfsFree(tmpPtr);
	}

	memcpy((void *)&(f->streamPtr[f->currentPos]), (void *)buffer,  numBytesToWrite);
