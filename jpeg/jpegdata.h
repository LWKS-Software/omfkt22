 ...
 */
	...
	boolean CCIR; /* use CCIR YCbCr levels? */

#if ! STANDARD_JPEG_Q
	boolean quant16FP;
#endif

	JSAMPLE *CCIRLumaInMap;
	JSAMPLE *CCIRChromaInMap;
 ...
 */
	...
	JSAMPLE *CCIRLumaOutMap;
	JSAMPLE *CCIRChromaOutMap;
 ...
 */
	int  image_width;	/* overall image width */
	int  image_height;	/* overall image height */
...
struct big_sarray_control { int  dummy; };
struct big_barray_control { int  dummy; };
#endif
#endif
...
struct External_methods_struct {
	...
	METHOD(JSAMPARRAY, alloc_small_sarray, (int  samplesperrow,
						int  numrows));
	METHOD(void, free_small_sarray, (JSAMPARRAY ptr));
	METHOD(JBLOCKARRAY, alloc_small_barray, (int  blocksperrow,
						 int  numrows));
	METHOD(void, free_small_barray, (JBLOCKARRAY ptr));
	METHOD(big_sarray_ptr, request_big_sarray, (int  samplesperrow,
						    int  numrows,
						    int  unitheight));
	METHOD(big_barray_ptr, request_big_barray, (int  blocksperrow,
						    int  numrows,
						    int  unitheight));
	METHOD(void, alloc_big_arrays, (int  extra_small_samples,
					int  extra_small_blocks,
	...
	int  max_memory_to_use;	/* maximum amount of memory to use */
};
...
EXTERN int  jround_up PP((int  a, int  b));
