...
static short descale16(int s, int quant);
...
static short descale16(int s, int quant)
{
   ...
   unsigned int  source;
	...
	neg = (s < 0);
	if (neg)
		source = (unsigned int )(-s);
	else
		source = (unsigned int )(s);

	result = (short)((source * (unsigned int )quant) >> 8);
...
huff_decode_mcu (decompress_info_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    ...
    /* pj yeah I don't like this either. */
#if ! STANDARD_JPEG_Q
	if (cinfo->quant16FP)
	 	(*block)[0] = descale16(s, quanttbl[0]);
	 else
#endif
...
#if ! STANDARD_JPEG_Q
	if (cinfo->quant16FP)
		(*block)[ZAG[k]] = descale16(s, quanttbl[k]);
	else
#endif
