...
METHODDEF void
ycc_rgb_convert (decompress_info_ptr cinfo, int num_rows, int  num_cols,
		 JSAMPIMAGE input_data, JSAMPIMAGE output_data)
{
  ...
  for (row = 0; row < num_rows; row++) {
    ...
    for (col = 0; col < num_cols; col++) {
      ...
      if (cinfo->CCIR && cinfo->CCIRLumaOutMap)
      {
      	if (y < 256)
      		y = cinfo->CCIRLumaOutMap[y];
      }
      cb = GETJSAMPLE(inptr1[col]);
      if (cinfo->CCIR && cinfo->CCIRChromaOutMap)
      {
      	if (cb < 256)
      		cb = cinfo->CCIRChromaOutMap[cb];
      }
      cr = GETJSAMPLE(inptr2[col]);
      if (cinfo->CCIR && cinfo->CCIRChromaOutMap)
      {
      	if (cr < 256)
      		cr = cinfo->CCIRChromaOutMap[cr];
      }
...
METHODDEF void
null_convert (decompress_info_ptr cinfo, int num_rows, int  num_cols,
	      JSAMPIMAGE input_data, JSAMPIMAGE output_data)
{
  short ci;
...
METHODDEF void
grayscale_convert (decompress_info_ptr cinfo, int num_rows, int  num_cols,
		   JSAMPIMAGE input_data, JSAMPIMAGE output_data)
{
