		     ...
		     JBLOCKROW output)
{
  ...
  int  blocks_in_row = compptr->downsampled_width / DCTSIZE;
  int  col;
...
#define QPTR(_z) ((Qptr[_z] >> 8) ? ((Qptr[_z] + 128) >> 8) : Qptr[_z]) 
#define COND_ASSIGN(_ac,_n,_z) \
   if ((ABS(output[col][_n] - (_ac))<<1) <= QPTR(_z)) \
       output[col][_n] = (JCOEF) (_ac)
