...
METHODDEF void
progress_monitor (compress_info_ptr cinfo, int  loopcounter, int  looplimit)
{
  /* do nothing */
}
...
LOCAL void
add_huff_table (compress_info_ptr cinfo, HUFF_TBL **htblptr, 
		const UINT8 *bits, int  bitsize,
		const UINT8 *val, int  valsize)
/* Define a Huffman table */
{
  if (*htblptr == NULL)
    *htblptr = (HUFF_TBL *) (*cinfo->emethods->alloc_small) (SIZEOF(HUFF_TBL));

  if (bitsize > SIZEOF((*htblptr)->bits))
    bitsize = SIZEOF((*htblptr)->bits);
  MEMCOPY((*htblptr)->bits, bits, bitsize);

/*  MEMCOPY((*htblptr)->bits, bits, SIZEOF((*htblptr)->bits)); */

  if (valsize > SIZEOF((*htblptr)->huffval))
    valsize = SIZEOF((*htblptr)->huffval);
...
/* IMPORTANT: these are only valid for 8-bit data precision! */
{
  ...
  add_huff_table(cinfo, &cinfo->dc_huff_tbl_ptrs[0],
		 dc_luminance_bits, SIZEOF(dc_luminance_bits),
		 dc_luminance_val, SIZEOF(dc_luminance_val));
  add_huff_table(cinfo, &cinfo->ac_huff_tbl_ptrs[0],
		 ac_luminance_bits, SIZEOF(ac_luminance_bits),
		 ac_luminance_val,  SIZEOF(ac_luminance_val));
  add_huff_table(cinfo, &cinfo->dc_huff_tbl_ptrs[1],
		 dc_chrominance_bits, SIZEOF(dc_chrominance_bits),
		 dc_chrominance_val, SIZEOF(dc_chrominance_val));
  add_huff_table(cinfo, &cinfo->ac_huff_tbl_ptrs[1],
		 ac_chrominance_bits, SIZEOF(ac_chrominance_bits),
		 ac_chrominance_val, SIZEOF(ac_chrominance_val));
}
 ...
 */
{
  ...
  for (i = 0; i < DCTSIZE2; i++) {
    temp = ((int ) basic_table[i] * scale_factor + 50L) / 100L;
...
#if ! STANDARD_JPEG_Q
  cinfo->quant16FP = FALSE;
#endif

  cinfo->CCIRLumaInMap = NULL;
  cinfo->CCIRChromaInMap = NULL;
  cinfo->CCIR = FALSE;
