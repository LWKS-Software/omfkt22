...
rgb_ycc_init (compress_info_ptr cinfo)
{
  ...
  pixel_row = (*cinfo->emethods->alloc_small_sarray)
		(cinfo->image_width, (int ) cinfo->input_components);
    ...
    for (col = 0; col < width; col++) {
      ...
      outptr0[col] = (JSAMPLE)
		((ctab[r+R_Y_OFF] + ctab[g+G_Y_OFF] + ctab[b+B_Y_OFF])
		 >> SCALEBITS);
	  if (cinfo->CCIR && cinfo->CCIRLumaInMap && (outptr0[col] < 256))
	      outptr0[col] = cinfo->CCIRLumaInMap[outptr0[col]];
	  ...
	  if (cinfo->CCIR && cinfo->CCIRChromaInMap && outptr1[col] < 256)
	      outptr1[col] = cinfo->CCIRChromaInMap[outptr1[col]]; 
	  ...
	  if (cinfo->CCIR && cinfo->CCIRChromaInMap && outptr2[col] < 256)
	      outptr2[col] = cinfo->CCIRChromaInMap[outptr2[col]]; 
