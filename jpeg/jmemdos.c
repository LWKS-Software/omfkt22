 ...
 * compiler memory model.  We assume "short" is 16 bits, "int " is 32.
 */
...
EXTERN short far jdos_seek PP((short handle, int  offset));
...
static int  total_used;		/* total FAR memory requested so far */
...
LOCAL boolean
open_file_store (backing_store_ptr info, int  total_bytes_needed)
{
  short handle;
...
typedef union {			/* either int  offset or real-mode pointer */
	int  offset;
	void far * ptr;
      } XMSPTR;
...
LOCAL boolean
open_xms_store (backing_store_ptr info, int  total_bytes_needed)
{
  XMScontext ctx;
...
LOCAL boolean
open_ems_store (backing_store_ptr info, int  total_bytes_needed)
{
  EMScontext ctx;
...
GLOBAL void
jopen_backing_store (backing_store_ptr info, int  total_bytes_needed)
{
